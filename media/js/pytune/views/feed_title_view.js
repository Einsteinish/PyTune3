PYTUNE.Views.FeedTitleView = Backbone.View.extend({
    ...
    initialize: function() {
        ...
        if (this.model.is_social() && !this.model.get('feed_title')) {
            var profile = PYTUNE.assets.user_profiles.get(this.model.get('user_id')) || {};
    ...
    render: function() {
          ...
          <% if (type == "story") { %>\
              ...
              <div class="NB-feedbar-options-container">\
                  <span class="NB-feedbar-options">\
                      ...
                      <%= PYTUNE.assets.view_setting(feed.id, "read_filter") %>\
                      &middot;\
                      <%= PYTUNE.assets.view_setting(feed.id, "order") %>\
                  </span>\
              </div>\
        ...
        if (PYTUNE.reader.flags.search || PYTUNE.reader.flags.searching) {
    ...
    extra_classes: function() {
        var feed = this.model;
        var extra_classes = '';
        var starred_feed = PYTUNE.assets.starred_feeds.get_feed(feed.id);
        ...
        if (feed.is_social()) {
            ...
            if (feed.get('subscription_user_id') == PYTUNE.Globals.user_id) {
                extra_classes += ' NB-feed-self-pytuneblog';
            }
        }
    ...
    render_counts: function() {
        ...
        this.counts_view = new PYTUNE.Views.UnreadCount({
            model: this.model,
    ...
    setup_tooltips: function() {
        if (this.options.type == 'story' && PYTUNE.assets.preference('show_tooltips')) {
    ...
    open: function(e, options) {
        ...
        if (this.model.get('has_exception') && this.model.get('exception_type') == 'feed') {
            PYTUNE.reader.open_feed_exception_modal(this.model.id);
        } else if (this.model.is_social()) {
            PYTUNE.reader.open_social_stories(this.model.id, {force: true, $feed: this.$el});
        } else if (this.model.is_starred()) {
            PYTUNE.reader.open_starred_stories({
                tag: this.model.tag_slug(),
                model: this.model,
                $feed: this.$el
            });
        } else {
            PYTUNE.reader.open_feed(this.model.id, {$feed: this.$el});
        }
    },
    ...
    open_feed_link: function(e) {
        e.preventDefault();
        e.stopPropagation();
        var dblclick_pref = PYTUNE.assets.preference('doubleclick_feed');
        ...
        if (dblclick_pref == "open_and_read") {
            PYTUNE.reader.mark_feed_as_read(this.model.id);
        }
    ...
    dblclick_mark_feed_as_read: function(e) {
        if (this.options.feed_chooser) return;
        if (PYTUNE.assets.preference('doubleclick_unread') == "ignore") return;
    ...
    mark_feed_as_read: function(e, days) {
        ...
        PYTUNE.reader.mark_feed_as_read(this.model.id, days);
    ...
    show_manage_menu: function(e) {
        ...
        PYTUNE.reader.show_manage_menu(feed_type, this.$el, {
    ...
    open_trainer: function() {
        if (!$('.NB-task-manage').hasClass('NB-disabled')) {
            PYTUNE.reader.open_feed_intelligence_modal(1, null, !PYTUNE.reader.flags.social_view);
        }
    },
