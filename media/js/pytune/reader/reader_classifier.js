PYTUNE.ReaderClassifierTrainer = function(options) {
    var defaults = {
        'width': 620,
        'training': true,
...
PYTUNE.ReaderClassifierFeed = function(feed_id, options) {
    var defaults = {
        'width': 620,
        'training': false,
        'feed_loaded': true,
...
PYTUNE.ReaderClassifierStory = function(story_id, feed_id, options) {
    var defaults = {
        'width': 620,
        'feed_loaded': true,
        modal_container_class: "NB-full-container NB-classifier-container"
...
var classifier_prototype = {
    ...
    load_previous_feed_in_trainer: function() {
        ...
        // PYTUNE.log(['load_previous_feed_in_trainer', this.trainer_iterator, trainer_data]);
    ...
    load_next_feed_in_trainer: function() {
        ...
        this.trainer_iterator += 1;
        var trainer_data = this.trainer_data[this.trainer_iterator];
        // PYTUNE.log(['load_next_feed_in_trainer', this.trainer_iterator, trainer_data]);
        ...
        this.$modal = $.make('div', { className: 'NB-modal-classifiers NB-modal NB-modal-trainer'}, [
            ...
            $.make('ol', { className: 'NB-trainer-points NB-classifiers' }, [
                ...
                $.make('li', [
                    ...
                    $.make('img', { className: 'NB-trainer-bullet', src: PYTUNE.Globals.MEDIA_URL + '/img/icons/circular/g_icn_focus.png'}),
                    ...
                    $.make('br'),
                    $.make('img', { className: 'NB-trainer-bullet', src: PYTUNE.Globals.MEDIA_URL + '/img/icons/circular/g_icn_hidden.png'}),
                    ' are stories you don\'t like'
                ]),
                $.make('li', [
                    // $.make('img', { src: PYTUNE.Globals.MEDIA_URL + '/img/reader/sample_menu.png', style: 'float: right', width: 176, height: 118 }),
                    $.make('b', 'Stop any time you like.'),
                    ' You can easily train individual stories as you read.'
                ])
            ]),
            (!PYTUNE.Globals.is_authenticated && $.make('div', { className: 'NB-trainer-not-authenticated' }, 'Please create an account and add sites you read. Then you can train them.')),
            $.make('div', { className: 'NB-modal-submit-bottom' }, [
                $.make('div', { className: 'NB-modal-submit' }, [
                    (!PYTUNE.Globals.is_authenticated && $.make('div', { className: 'NB-modal-submit-grey NB-modal-submit-button' }, 'Close')),
                    (PYTUNE.Globals.is_authenticated && $.make('div', { className: 'NB-modal-submit-begin NB-modal-submit-button NB-modal-submit-grey NB-disabled' }, 'Loading Training...'))
                ])
            ])
        ]);
    ...
    make_trainer_outro: function() {
        ...
        this.$modal = $.make('div', { className: 'NB-modal-classifiers NB-modal NB-modal-trainer'}, [
            ...
            $.make('ol', { className: 'NB-trainer-points' }, [
                $.make('li', [
                    $.make('img', { src: PYTUNE.Globals.MEDIA_URL + '/img/reader/sample_classifier_tag.png', style: 'float: right', width: 135 }),
                    $.make('b', 'You can change your opinions.'),
                    ' You can click the ',
                    $.make('img', { src: PYTUNE.Globals.MEDIA_URL + '/img/reader/thumbs_up.png', style: 'vertical-align: middle;padding: 0 8px 0 2px', width: 14, height: 20 }),
                    $.make('img', { src: PYTUNE.Globals.MEDIA_URL + '/img/reader/thumbs_down.png', style: 'vertical-align: top; padding: 0', width: 14, height: 20 }),
                    ' buttons next to stories as you read them.'
                ]),
                $.make('li', [
                    $.make('img', { src: PYTUNE.Globals.MEDIA_URL + '/img/reader/intelligence_slider_positive.png', style: 'float: right', width: 114, height: 29 }),
    ...
    make_modal_feed: function() {
        ...
        // PYTUNE.log(['Make feed', feed, this.feed_authors, this.feed_tags, this.options['feed_loaded']]);
    ...
    make_modal_story: function() {
        ...
        // PYTUNE.log(['Make Story', story, feed]);
    ...
    make_classifier: function(classifier_title, classifier_value, classifier_type, classifier_count, classifier) {
        var score = 0;
        // PYTUNE.log(['classifiers', this.user_classifiers, classifier_value, this.user_classifiers[classifier_type+'s']]);
    ...
    change_classifier: function($classifier, classifier_opinion) {
        ...
        // PYTUNE.log(['change_classifier', classifier_opinion, $classifier, $like.is(':checked'), $dislike.is(':checked')]);
    },
    ...
    handle_clicks: function(elem, e) {
        ...
        if (this.options['training']) {
            ...
            $.targetIs(e, { tagSelector: '.NB-modal-submit-end' }, function($t, $p){
                ...
                self.end();
                // PYTUNE.reader.open_feed(self.feed_id, true);
    ...
    save: function(keep_modal_open) {
        ...
        PYTUNE.assets.recalculate_story_scores(feed_id);
        PYTUNE.assets.stories.trigger('render:intelligence');
...
PYTUNE.ReaderClassifierStory.prototype = new PYTUNE.Modal;
PYTUNE.ReaderClassifierStory.prototype.constructor = PYTUNE.ReaderClassifierStory;
_.extend(PYTUNE.ReaderClassifierStory.prototype, classifier_prototype);

PYTUNE.ReaderClassifierFeed.prototype = new PYTUNE.Modal;
PYTUNE.ReaderClassifierFeed.prototype.constructor = PYTUNE.ReaderClassifierFeed;
_.extend(PYTUNE.ReaderClassifierFeed.prototype, classifier_prototype);

PYTUNE.ReaderClassifierTrainer.prototype = new PYTUNE.Modal;
PYTUNE.ReaderClassifierTrainer.prototype.constructor = PYTUNE.ReaderClassifierTrainer;
_.extend(PYTUNE.ReaderClassifierTrainer.prototype, classifier_prototype);
