PYTUNE.Views.Folder = Backbone.View.extend({
    ...
    render: function() {
        ...
        this.options.collapsed =  folder_title && _.contains(PYTUNE.Preferences.collapsed_folders, folder_title);
        ...
        if (!this.options.only_title) {
            var $feeds = _.compact(this.collection.map(function(item) {
                if (item.is_feed()) {
                    ...
                    if (!feed_title_view) {
                        feed_title_view = new PYTUNE.Views.FeedTitleView({
                            model: item.feed, 
                            type: 'feed',
                            depth: depth,
                    ...
                    if (!folder_view) {
                        folder_view = new PYTUNE.Views.Folder({
                            model: item,
                ...
                <% if (feedbar) { %>\
                    ...
                    <div class="NB-feedbar-options-container">\
                        <span class="NB-feedbar-options">\
                            ...
                            <%= PYTUNE.assets.view_setting("river:"+folder_title, "read_filter") %>\
                            &middot;\
                            <%= PYTUNE.assets.view_setting("river:"+folder_title, "order") %>\
                        </span>\
                    </div>\
    ...
    update_hidden: function() {
        ...
        if (!has_unreads && PYTUNE.assets.preference('hide_read_feeds')) {
    ...
    check_collapsed: function(options) {
        ...
        var collapsed = _.contains(PYTUNE.Preferences.collapsed_folders, this.options.folder_title);
    ...
    show_collapsed_folder_count: function(options) {
        ...
        this.folder_count = new PYTUNE.Views.UnreadCount({
    ...
    open: function(e) {
        ...
        PYTUNE.reader.open_river_stories(this.$el, this.model);
    },
    ...
    show_manage_menu: function(e) {
        ...
        PYTUNE.reader.show_manage_menu('folder', this.$el, {
    ...
    delete_folder: function() {
        ...
        if (_.contains(feed_ids_in_folder, PYTUNE.reader.active_feed)) {
            PYTUNE.reader.reset_feed();
            PYTUNE.reader.show_splash_page();
        }
    },
    ...
    collapse_folder: function(e, options) {
        ...
        if (options.force_collapse || 
            ...
            PYTUNE.log(["hiding folder", $children, this.collection, this.options.folder_title]);
            PYTUNE.assets.collapsed_folders(this.options.folder_title, true);
        ...
        // Showing / Expanding
            ...
            PYTUNE.log(["showing folder", this.collection, this.options.folder_title]);
            PYTUNE.assets.collapsed_folders(this.options.folder_title, false);
    ...
    highlighted_count: function() {
        var count = PYTUNE.assets.feeds.reduce(function(memo, item) {
    ...
    mark_folder_as_read: function(e, days_back) {
        PYTUNE.reader.mark_folder_as_read(this.model, days_back);
    ...
    show_hidden_story_titles: function() {
        PYTUNE.app.story_titles_header.show_hidden_story_titles();
    }
