(function($) {
    ...
    PYTUNE.Reader = Backbone.Router.extend({
        ...
        init: function(options) {
            ...
            PYTUNE.assets = new PYTUNE.AssetModel();
            ...
            this.$s = {
                ...
                $river_pytuneblogs_header: $('.NB-feeds-header-river-pytuneblogs'),
            ...
            if (PYTUNE.Flags['start_import_from_google_reader']) {
                this.start_import_from_google_reader();
            }
            PYTUNE.app.sidebar_header = new PYTUNE.Views.SidebarHeader({
                feed_collection: PYTUNE.assets.feeds,
                socialfeed_collection: PYTUNE.assets.social_feeds
            });
            PYTUNE.app.sidebar = new PYTUNE.Views.Sidebar();
            PYTUNE.app.feed_list = new PYTUNE.Views.FeedList({el: this.$s.$feed_list[0]});
            PYTUNE.app.story_titles = new PYTUNE.Views.StoryTitlesView({collection: PYTUNE.assets.stories});
            PYTUNE.app.story_list = new PYTUNE.Views.StoryListView({collection: PYTUNE.assets.stories});
            PYTUNE.app.original_tab_view = new PYTUNE.Views.OriginalTabView({collection: PYTUNE.assets.stories});
            PYTUNE.app.story_tab_view = new PYTUNE.Views.StoryTabView({collection: PYTUNE.assets.stories});
            PYTUNE.app.text_tab_view = new PYTUNE.Views.TextTabView({
                el: this.$s.$text_view,
                collection: PYTUNE.assets.stories
            });
            PYTUNE.app.feed_selector = new PYTUNE.Views.FeedSelector();
            PYTUNE.app.follow_requests_module = new PYTUNE.Views.FollowRequestsModule();
            PYTUNE.app.dashboard_search = new PYTUNE.Views.DashboardSearch();
            PYTUNE.app.taskbar_info = new PYTUNE.Views.ReaderTaskbarInfo().render();
            PYTUNE.app.story_titles_header = new PYTUNE.Views.StoryTitlesHeader();
        ...
        resize_window: function() {
            ...
            if (!this.flags['opening_feed'] && PYTUNE.app.story_unread_counter) {
                PYTUNE.app.story_unread_counter.center();
            }
            ...
            if (_.contains(['split', 'list', 'grid'],
                PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout'))) {
                PYTUNE.app.story_titles.fill_out();
            } else {
                PYTUNE.app.story_list.fill_out();
            }

            this.flags.fetch_story_locations_in_feed_view = this.flags.fetch_story_locations_in_feed_view ||
                                                            _.throttle(function() {
                                                                PYTUNE.app.story_list.reset_story_positions();
                                                            }, 2000);
        ...
        adjust_for_narrow_window: function() {
            ...
            var story_layout = PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout');
            var content_width;
            var $windows = this.$s.$body.add(this.$s.$feed_view)
                                        .add(this.$s.$story_titles)
                                        .add(this.$s.$text_view);
            if (story_layout == 'split') {
                north = PYTUNE.reader.layout.contentLayout.panes.north;
                center = PYTUNE.reader.layout.contentLayout.panes.center;
                west = PYTUNE.reader.layout.contentLayout.panes.west;
            } else {
                center = PYTUNE.reader.layout.rightLayout.panes.center;
            }
            if (center) {
                var center_width = center.width();
                var narrow = center_width < 780;
                if (PYTUNE.assets.preference('story_button_placement') == "bottom") {
                    narrow = true;
                }
        ...
        apply_resizable_layout: function(options) {
            ...
            $('.right-pane').removeClass('NB-story-pane-west')
                            ...
                            .toggleClass('NB-story-pane-'+story_anchor,
                                         PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout') == 'split');
            if (!options.right_side) {
            ...
            if (_.contains(['split', 'full'], 
                ...
                if (PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout') == 'full') {
        ...
        add_body_classes: function() {
            this.$s.$body.toggleClass('NB-is-premium',        PYTUNE.Globals.is_premium);
            this.$s.$body.toggleClass('NB-is-anonymous',      PYTUNE.Globals.is_anonymous);
            this.$s.$body.toggleClass('NB-is-authenticated',  PYTUNE.Globals.is_authenticated);
            ...
            this.$s.$body.removeClass('NB-story-layout-full')
                         ...
                         .addClass('NB-story-layout-'+PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout'));
        },
        ...
        hide_splash_page: function() {
            ...
            this.apply_tipsy_titles();
            if (PYTUNE.Globals.is_anonymous) {
        ...
        show_next_story: function(direction) {
            var story = PYTUNE.assets.stories.get_next_story(direction, {
            ...
            if (PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout') == 'full' &&
                ...
                var visible = PYTUNE.assets.stories.visible();
        ...
        show_next_unread_story: function() {
            ...
            if (this.flags['feed_list_showing_starred']) {
                ...
                var next_story = PYTUNE.assets.stories.get_next_unread_story();
                if (next_story) {
        ...
        open_next_unread_story_across_feeds: function(force_next_feed) {
            ...
            if (!unread_count && !this.flags['feed_list_showing_starred']) {
                if (this.flags.river_view && !this.flags.social_view) {
                    ...
                    var folder = PYTUNE.assets.folders.get_view($next_folder);
                    ...
                    if (PYTUNE.utils.is_feed_social(next_feed_id)) {
        ...
        show_last_unread_story: function() {
            ...
            if (unread_count) {
                var last_story = PYTUNE.assets.stories.get_last_unread_story(unread_count);
        ...
        select_story_in_feed: function() {
            ...
            var story = PYTUNE.assets.stories.get(story_id);
            // PYTUNE.log(['select_story_in_feed', story_id, story, this.story_view, this.counts['select_story_in_feed'], this.flags['no_more_stories']]);
        ...
        show_previous_story: function() {
            PYTUNE.assets.stories.select_previous_story();
        },
        ...
        show_next_feed: function(direction, $current_feed) {
            ...
            } else if (PYTUNE.utils.is_feed_social(next_feed_id)) {
                this.open_social_stories(next_feed_id, {force: true, $feed: $next_feed});
            } else {
                next_feed_id = parseInt(next_feed_id, 10);
                this.open_feed(next_feed_id, {force: true, $feed: $next_feed});
        ...
        get_next_feed: function(direction, $current_feed, options) {
            ...
            if (options.feed_id && $current_feed && $current_feed.length) {
                var current_feed = PYTUNE.assets.get_feed(options.feed_id);
                if (current_feed) {
        ...
        get_next_unread_folder: function(direction) {
            ...
            $folders = $folders.filter(function() {
                var $this = $(this);
                var folder_view = PYTUNE.assets.folders.get_view($current_folder);
        ...
        page_in_story: function(amount, direction) {
            ...
            var feed_view = PYTUNE.assets.preference('feed_view_single_story') && 
            ...
            if (PYTUNE.assets.preference('space_bar_action') == 'scroll_only') return;
            if (PYTUNE.assets.preference('space_bar_action') == 'next_unread_50') {
                page_height = page_height / 2 + 36;
            }
            
            if (!this.active_story || !this.active_story.get('selected')) {
                this.open_next_unread_story_across_feeds();                
            } else if (_.contains(['split', 'full'], PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout'))) {
                if (direction > 0) {
                    if (feed_view) {
            ...
            } else if (_.contains(['list', 'grid'], PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout'))) {
        ...
        scroll_in_story: function(amount, direction) {
            ...
            // PYTUNE.log(['scroll_in_story', this.$s.$story_pane, direction, amount]);
            if (_.contains(['list', 'grid'], PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout'))) {
        ...
        sort_feeds: function($feeds) {
            $('.feed', $feeds).tsort('', {sortFunction: PYTUNE.Collections.Folders.comparator});
        ...
        load_sortable_feeds: function() {
            ...
            this.$s.$feed_list.sortable({
                ...
                start: function(e, ui) {
                    ...
                    PYTUNE.app.feed_list.start_sorting();
                ...
                stop: function(e, ui) {
                    ...
                    PYTUNE.app.feed_list.end_sorting();
        ...
        save_feed_order: function() {
            ...
            // PYTUNE.log(['Save new folder/feed order', {'combined': combined_folders}]);
        ...
        open_dialog_after_feeds_loaded: function(options) {
            options = options || {};
            if (!PYTUNE.Globals.is_authenticated) return;
            
            if (!PYTUNE.assets.folders.length ||
                !PYTUNE.assets.preference('has_setup_feeds')) {
                if (options.delayed_import || this.flags.delayed_import) {
                    this.setup_ftux_add_feed_callout("Check your email...");
                } else if (options.finished_intro || PYTUNE.assets.preference('has_setup_feeds')) {
                    this.setup_ftux_add_feed_callout();
                } else if (!PYTUNE.intro || !PYTUNE.intro.flags.open) {
                    _.defer(_.bind(this.open_intro_modal, this), 100);
                }
            } else if (!PYTUNE.assets.flags['has_chosen_feeds'] &&
                       PYTUNE.assets.folders.length) {
                if (PYTUNE.Globals.is_premium) {
                    this.model.save_feed_chooser(null, function() {
                        PYTUNE.reader.hide_feed_chooser_button();
                        PYTUNE.assets.load_feeds();
                    });
                } else {
                    _.defer(_.bind(this.open_feedchooser_modal, this), 100);
                }
            } else if (!PYTUNE.Globals.is_premium &&
                       PYTUNE.assets.feeds.active().length > 64) {
                _.defer(_.bind(this.open_feedchooser_modal, this), 100);
            }
        },
        ...
        reset_feed: function(options) {
            ...
            this.$s.$river_pytuneblogs_header.removeClass('NB-selected');
            ...
            PYTUNE.assets.stories.reset();
            PYTUNE.app.feed_selector.hide_feed_selector();
            PYTUNE.app.original_tab_view.unload_feed_iframe();
            PYTUNE.app.story_tab_view.unload_story_iframe();
            PYTUNE.app.text_tab_view.unload();
        },
        ...
        reload_feed: function(options) {
            ...
            if (this.flags['starred_view'] && this.flags['starred_tag']) {
                ...
                this.open_read_stories(options);
                       ...
                       this.active_feed == 'river:pytuneblogs') {
                this.open_river_pytuneblogs_stories();
            } else if (this.flags['social_view'] && 
                       this.active_feed == 'river:global') {
                this.open_river_pytuneblogs_stories({'global': true});
            } else if (this.flags['social_view']) {
        ...
        open_feed: function(feed_id, options) {
            ...
            if (options.try_feed || feed) {
                ...
                if (PYTUNE.app.story_unread_counter) {
                    PYTUNE.app.story_unread_counter.remove();
                }
                
                PYTUNE.app.taskbar_info.hide_stories_error();
                ...
                if (PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout') == 'full') {
                    PYTUNE.app.story_list.show_loading(options);
                } else {
                    PYTUNE.app.story_titles.show_loading(options);
                }

                _.delay(_.bind(function() {
                    if (!options.delay || feed.id == self.next_feed) {
                        this.model.load_feed(feed.id, 1, true, $.rescope(this.post_open_feed, this), 
                                             PYTUNE.app.taskbar_info.show_stories_error);
                    }
                }, this), options.delay || 0);

                if (_.contains(['split', 'full'], PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout')) && 
                    (!this.story_view || this.story_view == 'page')) {
                    _.delay(_.bind(function() {
                        if (!options.delay || feed.id == this.next_feed) {
                            PYTUNE.app.original_tab_view.load_feed_iframe(feed.id);
                        }
                    }, this), options.delay || 0);
                } else {
                    PYTUNE.app.original_tab_view.unload_feed_iframe();
                ...
                if (!options.silent) {
                    ...
                    if (!_.string.include(window.location.pathname, url)) {
                        PYTUNE.log(["Navigating to url", url]);
                        PYTUNE.router.navigate(url);
                    }
                }
            }
        },
        ...
        post_open_feed: function(e, data, first_load) {
            if (!data) {
                PYTUNE.log(["No data from feed, trying again..."]);
            ...
            PYTUNE.app.story_titles_header.show_feed_hidden_story_title_indicator(first_load);
            ...
            if (first_load) {
                ...
                if (_.contains(['story', 'text'], this.story_view) &&
                    !this.active_story &&
                    _.contains(['split', 'full'], PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout')) &&
                    !this.counts['find_next_unread_on_page_of_feed_stories_load']) {
                    if (this.story_view == 'text') {
                        PYTUNE.app.text_tab_view.show_explainer_single_story_mode();
                    } else if (this.story_view == 'story') {
                        PYTUNE.app.story_tab_view.show_explainer_single_story_mode();                        
                    }
                }
            }

            PYTUNE.app.taskbar_info.hide_stories_progress_bar();
            if (PYTUNE.Globals.is_anonymous) {
        ...
        set_correct_story_view_for_feed: function(feed_id, view) {
            ...
            var feed = PYTUNE.assets.get_feed(feed_id);
            var $original_tabs = $('.task_view_page, .task_view_story');
            var $page_tab = $('.task_view_page');
            view = view || PYTUNE.assets.view_setting(feed_id);
            ...
            if (feed && 
                (feed.get('disabled_page') ||
                 PYTUNE.utils.is_url_iframe_buster(feed.get('feed_link')))) {
                view = 'feed';
        ...
        switch_story_layout: function(story_layout) {
            var feed_layout = PYTUNE.assets.view_setting(this.active_feed, 'layout');
            ...
            if (story_layout == 'list') {
                ...
                PYTUNE.app.story_list.render();
                if (this.active_story) {
                    PYTUNE.app.story_list.show_only_selected_story();
                    this.active_story.story_title_view.toggle_selected();
            ...
            _.defer(function() {
                PYTUNE.app.story_titles.scroll_to_selected_story();
                PYTUNE.app.story_list.scroll_to_selected_story();
                PYTUNE.app.feed_list.scroll_to_selected();
                if (_.contains(['split', 'list', 'grid'], 
                    PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout'))) {
                    PYTUNE.app.story_titles.fill_out();
                } else {
        ...
        open_starred_stories: function(options) {
            ...
            if (options.tag && !options.model) {
                var model = PYTUNE.assets.starred_feeds.detect(function(feed) {
            ...
            this.setup_mousemove_on_views();
            this.make_feed_title_in_stories();  
            PYTUNE.app.feed_list.scroll_to_show_selected_folder();
            ...
            this.model.fetch_starred_stories(1, this.flags['starred_tag'], _.bind(this.post_open_starred_stories, this), 
                                             PYTUNE.app.taskbar_info.show_stories_error, true);
        ...
        post_open_starred_stories: function(data, first_load) {
            ...
            // PYTUNE.log(['post_open_starred_stories', data.stories.length, first_load]);
            this.flags['opening_feed'] = false;
        ...
        post_open_read_stories: function(data, first_load) {
            if (this.active_feed == 'read') {
                // PYTUNE.log(['post_open_read_stories', data.stories.length, first_load]);
                this.flags['opening_feed'] = false;
                if (this.counts['select_story_in_feed'] || this.flags['select_story_in_feed']) {
                    this.select_story_in_feed();
                }
        ...
        open_river_stories: function($folder, folder, options) {
            ...
            var folder_view = PYTUNE.assets.folders.get_view($folder) ||
            ...
            this.active_folder = folder || PYTUNE.assets.folders;
            ...
            if (PYTUNE.reader.flags.search) visible_only = false;
            if (PYTUNE.reader.flags.feed_list_showing_starred) visible_only = false;
            var feeds;
            if (visible_only) {
            ...
            PYTUNE.app.taskbar_info.hide_stories_error();
            // PYTUNE.app.taskbar_info.show_stories_progress_bar(feeds.length);
            this.model.fetch_river_stories(this.active_feed, feeds, 1, 
                _.bind(this.post_open_river_stories, this), PYTUNE.app.taskbar_info.show_stories_error, true);
        },
        
        post_open_river_stories: function(data, first_load) {
            // PYTUNE.log(['post_open_river_stories', data, this.active_feed]);
            if (!data) {
              return PYTUNE.app.taskbar_info.show_stories_error(data);
            }
            ...
            if (this.active_feed && _.isString(this.active_feed) &&
                ...
                // PYTUNE.app.taskbar_info.hide_stories_progress_bar(_.bind(function() {
                    if (first_load) {
                        this.position_mouse_indicator();
                        this.make_story_titles_pane_counter();
                    }
        ...
        open_river_pytuneblogs_stories: function(options) {
            options = options || {};
            var $story_titles = this.$s.$story_titles;
            var folder_title = options.global ? "Global Pytuneblogs" : "Pytuneblogs";
            
            this.reset_feed(options);
            this.hide_splash_page();
            
            this.active_feed = options.global ? 'river:global' : 'river:pytuneblogs';
            ...
            if (options.global) {
                ...
                this.$s.$river_pytuneblogs_header.addClass('NB-selected');
            }
            ...
            this.flags.global_pytuneblogs = options.global;
            ...
            PYTUNE.app.taskbar_info.hide_stories_error();
            PYTUNE.app.taskbar_info.show_stories_progress_bar(100); // Assume 100 followees for popular
            this.model.fetch_river_pytuneblogs_stories(this.active_feed, 1, 
                {'global': this.flags.global_pytuneblogs},
                _.bind(this.post_open_river_pytuneblogs_stories, this), 
                PYTUNE.app.taskbar_info.show_stories_error, true);
        },
        
        post_open_river_pytuneblogs_stories: function(data, first_load) {
            // PYTUNE.log(['post_open_river_stories', data, this.active_feed]);
            if (!data) {
              return PYTUNE.app.taskbar_info.show_stories_error(data);
            }
        ...
        open_social_stories: function(feed_id, options) {
            // PYTUNE.log(["open_social_stories", feed_id, options]);
            options = options || {};
            ...
            // TODO: Only make feed the default for pytuneblogs, not overriding an explicit pref.
            this.switch_taskbar_view(this.story_view);
            this.switch_story_layout();
            this.setup_mousemove_on_views();
            ...
            PYTUNE.app.taskbar_info.hide_stories_error();
            PYTUNE.app.taskbar_info.show_stories_progress_bar();
            this.model.fetch_social_stories(this.active_feed, 1, 
                _.bind(this.post_open_social_stories, this), PYTUNE.app.taskbar_info.show_stories_error, true);
            
            if (this.story_view == 'page') {
                _.delay(_.bind(function() {
                    if (!options.delay || feed.id == this.next_feed) {
                        PYTUNE.app.original_tab_view.load_feed_iframe();
                    }
                }, this), options.delay || 0);
            } else {
                this.flags['iframe_prevented_from_loading'] = true;
            ...
            if (!options.silent && feed.get('feed_title')) {
                ...
                if (!_.string.include(window.location.pathname, url)) {
                    ...
                    PYTUNE.log(["Navigating to social", url, window.location.pathname]);
                    PYTUNE.router.navigate(url, params);
                }
            } else if (!feed.get('feed_title')) {
                PYTUNE.log(["No feed title on social", feed]);
                PYTUNE.router.navigate('');
            }
        },
        
        post_open_social_stories: function(data, first_load) {
            // PYTUNE.log(['post_open_river_stories', data, this.active_feed, this.flags['select_story_in_feed']]);
            if (!data) {
              return PYTUNE.app.taskbar_info.show_stories_error(data);
            }
            
            if (this.active_feed && PYTUNE.utils.is_feed_social(this.active_feed)) {
                this.flags['opening_feed'] = false;
                // this.show_story_titles_above_intelligence_level({'animate': false});
                PYTUNE.app.story_titles_header.show_feed_hidden_story_title_indicator(first_load);
                this.flags['story_titles_loaded'] = true;
        ...
        switch_to_correct_view: function(options) {
            options = options || {};
            // PYTUNE.log(['Found story', this.story_view, options.found_story_in_page, this.flags['page_view_showing_feed_view'], this.flags['feed_view_showing_story_view']]);
            ...
            if (options.story_not_found) {
                ...
                if (this.story_view == 'page' && !this.flags['page_view_showing_feed_view']) {
                    // PYTUNE.log(['turn on feed view', this.flags['page_view_showing_feed_view'], this.flags['feed_view_showing_story_view']]);
                    ...
                    PYTUNE.app.story_list.show_stories_preference_in_feed_view();
                }
            } else if (_.contains(['list', 'grid'], 
                PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout')) && 
                ...
                // PYTUNE.log(['turn off feed view', this.flags['page_view_showing_feed_view'], this.flags['feed_view_showing_story_view']]);
                this.flags['page_view_showing_feed_view'] = false;
                this.flags['feed_view_showing_story_view'] = false;
                this.flags['temporary_story_view'] = false;
                this.switch_taskbar_view('page');
            } else if (this.flags['feed_view_showing_story_view']) {
                // PYTUNE.log(['turn off story view', this.flags['page_view_showing_feed_view'], this.flags['feed_view_showing_story_view']]);
                this.flags['page_view_showing_feed_view'] = false;
                this.flags['feed_view_showing_story_view'] = false;
                this.flags['temporary_story_view'] = false;
        ...
        mark_active_story_read: function() {
            ...
            if (this.active_story && !this.active_story.get('read_status')) {
                PYTUNE.assets.stories.mark_read(story, {skip_delay: true});
            } else if (this.active_story && this.active_story.get('read_status')) {
                PYTUNE.assets.stories.mark_unread(story);
            }
        },
        
        maybe_mark_all_as_read: function() {
            if (_.contains(['river:pytuneblogs', 'river:global'], this.active_feed)) {
                return;
            } else if (this.flags.social_view) {
                this.mark_feed_as_read();
            } else if (this.flags.river_view) {
                if (this.active_feed == 'river:' && PYTUNE.assets.preference('mark_read_river_confirm')) {
        ...
        mark_feed_as_read: function(feed_id, days_back, direction) {
            feed_id = feed_id || this.active_feed;
            var cutoff_timestamp = PYTUNE.utils.days_back_to_timestamp(days_back);
            if (!days_back && this.model.stories.length && 
                this.model.stories.first().get('story_feed_id') == feed_id &&
                PYTUNE.assets.view_setting(feed_id, 'order') == 'newest') {
            ...
            if (!direction && PYTUNE.assets.preference('markread_nextfeed') == 'nextfeed' && 
                PYTUNE.reader.active_feed == feed_id) {
        ...
        mark_folder_as_read: function(folder, days_back, direction) {
            ...
            if (!direction && PYTUNE.assets.preference('markread_nextfeed') == 'nextfeed' &&
                PYTUNE.reader.active_folder == folder) {
                this.show_next_feed(1);
            }
        },
        ...
        open_story_trainer: function(story_id, feed_id, options) {
            ...
            if (story_id && feed_id) {
                ...
                PYTUNE.classifier = new PYTUNE.ReaderClassifierStory(story_id, feed_id, options);
            }
        },
        ...
        send_story_to_facebook: function(story_id) {
            var story = this.model.get_story(story_id);
            var url = 'http://www.facebook.com/sharer.php?src=pytune&v=3.14159265&i=1.61803399';
            var facebook_url = [
              url,
              '&u=',
              encodeURIComponent(story.get('story_permalink')),
        ...
        send_story_to_buffer: function(story_id) {
            var story = this.model.get_story(story_id);
            var url = 'https://bufferapp.com/add?source=pytune&';
            var buffer_url = [
              url,
              'url=',
              encodeURIComponent(story.get('story_permalink')),
        ...
        send_story_to_email: function(story) {
            PYTUNE.reader_send_email = new PYTUNE.ReaderSendEmail(story);
            PYTUNE.assets.stories.mark_read(story, {skip_delay: true});
        },
        ...
        make_story_titles_pane_counter: function(options) {
            if (PYTUNE.app.story_unread_counter) {
                PYTUNE.app.story_unread_counter.remove();
                PYTUNE.app.story_unread_counter.destroy();
            }
            ...
            if (feed) {
                PYTUNE.app.story_unread_counter = new PYTUNE.Views.UnreadCount({
                    model: feed
                }).render();
            } else if (folder) {
                var collection;
                if (!folder.folder_view) {
                    // River pytuneblog gets a special collection
                    collection = PYTUNE.assets.folders;
                } else {
            ...
            if (options.fade && PYTUNE.app.story_unread_counter) {
                PYTUNE.app.story_unread_counter.$el.css({'opacity': 0});
                this.$s.$story_taskbar.append(PYTUNE.app.story_unread_counter.$el);
                _.delay(function() {
                    PYTUNE.app.story_unread_counter.center();
                    PYTUNE.app.story_unread_counter.$el.animate({
                        'opacity': .2
                    }, {'duration': 600, 'queue': false});
                }, 200);
            } else if (PYTUNE.app.story_unread_counter) {
                this.$s.$story_taskbar.append(PYTUNE.app.story_unread_counter.$el);
                _.delay(function() {
                    PYTUNE.app.story_unread_counter.center();
                    PYTUNE.app.story_unread_counter.$el.css({'opacity': .2});
                }, 200);
            }
        },
        ...
        load_page_of_feed_stories: function(options) {
            ...
            if (this.flags['starred_view']) {
                this.model.fetch_starred_stories(this.counts['page'], this.flags['starred_tag'], _.bind(this.post_open_starred_stories, this),
                                                 PYTUNE.app.taskbar_info.show_stories_error, false);
            } else if (this.active_feed == 'read') {
                this.model.fetch_read_stories(this.counts['page'], _.bind(this.post_open_read_stories, this),
                                                 PYTUNE.app.taskbar_info.show_stories_error, false);
            } else if (this.flags['social_view'] && _.contains(['river:pytuneblogs', 'river:global'], this.active_feed)) {
                this.model.fetch_river_pytuneblogs_stories(this.active_feed,
                                                         this.counts['page'],
                                                         {'global': this.flags.global_pytuneblogs},
                                                         _.bind(this.post_open_river_pytuneblogs_stories, this),
                                                         PYTUNE.app.taskbar_info.show_stories_error, false);
                ...
                this.model.load_feed(feed_id, this.counts['page'], false, 
                                     $.rescope(this.post_open_feed, this), PYTUNE.app.taskbar_info.show_stories_error);                                 
            }
        },
        
        make_feed_title_in_stories: function(options) {
            if ((this.flags.search || this.flags.searching)
                && PYTUNE.app.story_titles_header.search_has_focus()) {
                console.log(["make_feed_title_in_stories not destroying", this.flags.search]);
                return;
            }
            
            if (PYTUNE.app.story_titles_header) {
                PYTUNE.app.story_titles_header.remove();
            }

            PYTUNE.app.story_titles_header.render({
                feed_id: this.active_feed,
                layout: PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout')
            });
        },
        ...
        open_feed_intelligence_modal: function(score, feed_id, feed_loaded) {
            ...
            if (feed_id) {
                PYTUNE.classifier = new PYTUNE.ReaderClassifierFeed(feed_id, {
                    'score': score,
        ...
        open_trainer_modal: function(score) {
            ...
            // PYTUNE.classifier = new PYTUNE.ReaderClassifierFeed(feed_id, {'score': score});
            PYTUNE.classifier = new PYTUNE.ReaderClassifierTrainer({'score': score});
        },    
            
        open_friends_modal: function() {
            PYTUNE.assets.preference('has_found_friends', true);
            PYTUNE.reader.check_hide_getting_started();

            PYTUNE.reader_friends = new PYTUNE.ReaderFriends();
        },
        
        open_profile_editor_modal: function() {
            PYTUNE.reader_profile_editor = new PYTUNE.ReaderProfileEditor();
        },
        
        open_recommend_modal: function(feed_id) {
            PYTUNE.recommend_feed = new PYTUNE.ReaderRecommendFeed(feed_id);
        },
        
        open_tutorial_modal: function() {
            PYTUNE.tutorial = new PYTUNE.ReaderTutorial();
        },
        
        open_intro_modal: function(options) {
            PYTUNE.intro = new PYTUNE.ReaderIntro(options);
        },
        
        open_user_admin_modal: function(options) {
            $.modal.close(function() {
                PYTUNE.user_admin = new PYTUNE.ReaderUserAdmin(options);
            });
        },
        
        open_story_options_popover: function() {
            PYTUNE.StoryOptionsPopover.create({
        ...
        apply_story_styling: function(reset_stories) {
            ...
            $body.addClass('NB-theme-'+PYTUNE.Preferences['story_styling']);
            ...
            $body.addClass('NB-theme-size-' + PYTUNE.Preferences['story_size']);
            ...
            $body.addClass('NB-theme-feed-size-' + PYTUNE.Preferences['feed_size']);
            ...
            $body.addClass('NB-line-spacing-' + PYTUNE.Preferences['story_line_spacing']);
        ...
        switch_taskbar_view: function(view, options) {
            options = options || {};
            // PYTUNE.log(['switch_taskbar_view', view, options.skip_save_type]);
            var self = this;
            ...
            if (view == 'page') {
                // PYTUNE.log(["iframe_prevented_from_loading", this.flags['iframe_prevented_from_loading']]);
                if (this.flags['iframe_prevented_from_loading']) {
                    PYTUNE.app.original_tab_view.load_feed_iframe();
                }
                PYTUNE.app.original_tab_view.scroll_to_selected_story(this.active_story, {
                    immediate: true,
                ...
                PYTUNE.app.story_list.scroll_to_selected_story(this.active_story, {
                    immediate: true,
                    only_if_hidden: options.resize
                });
                PYTUNE.app.story_list.show_stories_preference_in_feed_view();
                PYTUNE.app.story_titles.scroll_to_selected_story(this.active_story);
                ...
                if (_.contains(['split', 'full'], 
                               PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout'))) {
                    PYTUNE.app.text_tab_view.fetch_and_render();
                    if (!this.active_story) {
                        PYTUNE.app.text_tab_view.show_explainer_single_story_mode();
                    }
                ...
                if (!this.active_story) {
                    ...
                    PYTUNE.app.story_tab_view.open_story();
                }
            }
        ...
        open_add_feed_modal: function(options) {
            ...
            PYTUNE.add_feed = PYTUNE.ReaderAddFeed.create(options);
        },
        
        open_manage_feed_modal: function(feed_id) {
            feed_id = feed_id || this.active_feed;
            
            PYTUNE.manage_feed = new PYTUNE.ReaderManageFeed(feed_id);
        },

        open_mark_read_modal: function(options) {
            PYTUNE.mark_read = new PYTUNE.ReaderMarkRead(options);
        },

        open_keyboard_shortcuts_modal: function() {
            PYTUNE.keyboard = new PYTUNE.ReaderKeyboard();
        },
                
        open_goodies_modal: function() {
            PYTUNE.goodies = new PYTUNE.ReaderGoodies();
        },
                        
        open_preferences_modal: function() {
            PYTUNE.preferences = new PYTUNE.ReaderPreferences();
        },
                        
        open_account_modal: function(options) {
            PYTUNE.account = new PYTUNE.ReaderAccount(options);
        },
        
        open_feedchooser_modal: function(options) {
            PYTUNE.feedchooser = new PYTUNE.ReaderFeedchooser(options);
        },
        
        open_organizer_modal: function(options) {
            PYTUNE.organizer = new PYTUNE.ReaderOrganizer(options);
        },
        
        open_feed_exception_modal: function(feed_id, options) {
            feed_id = feed_id || this.active_feed;
            
            PYTUNE.feed_exception = new PYTUNE.ReaderFeedException(feed_id, options);
        },
        
        open_feed_statistics_modal: function(feed_id) {
            feed_id = feed_id || this.active_feed;
            
            PYTUNE.statistics = new PYTUNE.ReaderStatistics(feed_id);
        },
        
        open_social_profile_modal: function(user_id) {
            if (!user_id) user_id = PYTUNE.Globals.user_id;
            if (_.string.contains(user_id, 'social:')) {
                user_id = parseInt(user_id.replace('social:', ''), 10);
            }
            PYTUNE.social_profile = new PYTUNE.ReaderSocialProfile(user_id);
        },
        
        close_social_profile: function() {
            if (PYTUNE.social_profile) {
                PYTUNE.social_profile.close();
            }
        },
        
        close_interactions_popover: function() {
            PYTUNE.InteractionsPopover.close();
        },
        ...
        close_story_titles_pane: function(update_layout) {
            var story_anchor = this.model.preference('story_pane_anchor');
            if (update_layout) {
                PYTUNE.reader.layout.contentLayout.hide(story_anchor);
            }
            this.resize_window();
            this.flags['story_titles_closed'] = true;
        },
        ...
        open_story_titles_pane: function(update_layout) {
            var story_anchor = this.model.preference('story_pane_anchor');
            if (update_layout) {
                PYTUNE.reader.layout.contentLayout.open(story_anchor);
            }
            this.resize_window();
            this.flags['story_titles_closed'] = false;
            _.defer(function() {
        ...
        make_manage_menu: function(type, feed_id, story_id, inverse, $item) {
            var $manage_menu;
            // PYTUNE.log(["make_manage_menu", type, feed_id, story_id, inverse, $item]);

            if (type == 'site') {
                var show_chooser = !PYTUNE.Globals.is_premium && PYTUNE.Globals.is_authenticated;
                $manage_menu = $.make('ul', { className: 'NB-menu-manage' }, [
                    $.make('li', { className: 'NB-menu-manage-site-info' }, [
                        $.make('div', { className: 'NB-menu-manage-image' }),
                        $.make('span', { className: 'NB-menu-manage-title' }, "Manage PyTune")
                    ...
                    (PYTUNE.Globals.is_premium && $.make('li', { className: 'NB-menu-item NB-menu-manage-feedchooser' }, [
                        $.make('div', { className: 'NB-menu-manage-image' }),
                    ...
                    $.make('li', { className: 'NB-menu-item NB-menu-manage-profile-editor' }, [
                        $.make('div', { className: 'NB-menu-manage-image' }),
                        $.make('div', { className: 'NB-menu-manage-title' }, 'Profile &amp; Pytuneblog')
                    ]),
                ...
                $manage_menu = $.make('ul', { className: 'NB-menu-manage NB-menu-manage-feed' }, [
                    ...
                    $.make('li', { className: 'NB-menu-separator' }),
                    (PYTUNE.Globals.is_admin && $.make('li', { className: 'NB-menu-item NB-menu-manage-feed-recommend' }, [
                        $.make('div', { className: 'NB-menu-manage-image' }),
                        $.make('div', { className: 'NB-menu-manage-title' }, 'Recommend this site')
                    ])),
                    (PYTUNE.Globals.is_admin && $.make('li', { className: 'NB-menu-separator' })),
                ...
                $manage_menu = $.make('ul', { className: 'NB-menu-manage NB-menu-manage-feed' }, [
                    ...
                    $.make('li', { className: 'NB-menu-separator' }),
                    (feed.get('user_id') != PYTUNE.Globals.user_id && $.make('li', { className: 'NB-menu-item NB-menu-manage-delete NB-menu-manage-socialfeed-delete' }, [
                        $.make('div', { className: 'NB-menu-manage-image' }),
                ...
                $manage_menu = $.make('ul', { className: 'NB-menu-manage NB-menu-manage-folder' }, [
                    ...
                    $.make('li', { className: 'NB-menu-subitem NB-menu-manage-confirm NB-menu-manage-folder-move-confirm NB-modal-submit' }, [
                        $.make('div', { className: 'NB-menu-manage-confirm-position'}, [
                            ...
                            $.make('div', { className: 'NB-menu-manage-image' }),
                            $.make('div', { className: 'NB-add-folders' }, PYTUNE.utils.make_folders())
                        ])
                    ]),
                ...
                var shared_title = story.get('shared') ? 'Shared' : 'Share to your Pytuneblog';
                var order        = PYTUNE.assets.view_setting(this.active_feed, 'order');
                story.story_share_menu_view = new PYTUNE.Views.StoryShareView({
                    model: story
                });
                ...
                $manage_menu = $.make('ul', { className: 'NB-menu-manage NB-menu-manage-story ' + starred_class + shared_class }, [
                    ...
                    $.make('li', { className: 'NB-menu-item NB-menu-manage-story-thirdparty' }, [
                        (PYTUNE.Preferences['story_share_facebook'] && $.make('div', { className: 'NB-menu-manage-thirdparty-icon NB-menu-manage-thirdparty-facebook'}).bind('mouseenter', _.bind(function(e) {
                        ...
                        (PYTUNE.Preferences['story_share_twitter'] && $.make('div', { className: 'NB-menu-manage-thirdparty-icon NB-menu-manage-thirdparty-twitter'}).bind('mouseenter', _.bind(function(e) {
                        ...
                        (PYTUNE.Preferences['story_share_readitlater'] && $.make('div', { className: 'NB-menu-manage-thirdparty-icon NB-menu-manage-thirdparty-readitlater'}).bind('mouseenter', _.bind(function(e) {
                        ...
                        (PYTUNE.Preferences['story_share_tumblr'] && $.make('div', { className: 'NB-menu-manage-thirdparty-icon NB-menu-manage-thirdparty-tumblr'}).bind('mouseenter', _.bind(function(e) {
                        ...
                        (PYTUNE.Preferences['story_share_blogger'] && $.make('div', { className: 'NB-menu-manage-thirdparty-icon NB-menu-manage-thirdparty-blogger'}).bind('mouseenter', _.bind(function(e) {
                        ...
                        (PYTUNE.Preferences['story_share_delicious'] && $.make('div', { className: 'NB-menu-manage-thirdparty-icon NB-menu-manage-thirdparty-delicious'}).bind('mouseenter', _.bind(function(e) {
                        ...
                        (PYTUNE.Preferences['story_share_pinboard'] && $.make('div', { className: 'NB-menu-manage-thirdparty-icon NB-menu-manage-thirdparty-pinboard'}).bind('mouseenter', _.bind(function(e) {
                        ...
                        (PYTUNE.Preferences['story_share_pinterest'] && $.make('div', { className: 'NB-menu-manage-thirdparty-icon NB-menu-manage-thirdparty-pinterest'}).bind('mouseenter', _.bind(function(e) {
                        ...
                        (PYTUNE.Preferences['story_share_buffer'] && $.make('div', { className: 'NB-menu-manage-thirdparty-icon NB-menu-manage-thirdparty-buffer'}).bind('mouseenter', _.bind(function(e) {
                        ...
                        (PYTUNE.Preferences['story_share_diigo'] && $.make('div', { className: 'NB-menu-manage-thirdparty-icon NB-menu-manage-thirdparty-diigo'}).bind('mouseenter', _.bind(function(e) {
                        ...
                        (PYTUNE.Preferences['story_share_kippt'] && $.make('div', { className: 'NB-menu-manage-thirdparty-icon NB-menu-manage-thirdparty-kippt'}).bind('mouseenter', _.bind(function(e) {
                        ...
                        (PYTUNE.Preferences['story_share_evernote'] && $.make('div', { className: 'NB-menu-manage-thirdparty-icon NB-menu-manage-thirdparty-evernote'}).bind('mouseenter', _.bind(function(e) {
                        ...
                        (PYTUNE.Preferences['story_share_googleplus'] && $.make('div', { className: 'NB-menu-manage-thirdparty-icon NB-menu-manage-thirdparty-googleplus'}).bind('mouseenter', _.bind(function(e) {
                        ...
                        (PYTUNE.Preferences['story_share_instapaper'] && $.make('div', { className: 'NB-menu-manage-thirdparty-icon NB-menu-manage-thirdparty-instapaper'}).bind('mouseenter', _.bind(function(e) {
                        ...
                        (PYTUNE.Preferences['story_share_readability'] && $.make('div', { className: 'NB-menu-manage-thirdparty-icon NB-menu-manage-thirdparty-readability'}).bind('mouseenter', _.bind(function(e) {
        ...
        show_manage_menu: function(type, $item, options) {
            ...
            if (type == 'site') {
                ...
                var left, top;
                // PYTUNE.log(['menu open', $item, inverse, toplevel, type]);
                if (inverse) {
                    var $align = $item;
                    if (type == 'feed') {
                        left = toplevel ? 2 : -22;
            ...
            if (type == 'story') {
                var share = _.bind(function(e) {
                    e.preventDefault();
                    var story = PYTUNE.assets.get_story(story_id);
        ...
        hide_manage_menu: function(type, $item, animate) {
            ...
            // PYTUNE.log(['hide_manage_menu', type, $item, animate, $manage_menu_container.css('opacity')]);
        ...
        manage_menu_unfollow_feed: function(feed, $feed) {
            ...
            this.model.unfollow_user(feed_id, function() {
                PYTUNE.app.feed_list.make_social_feeds();
            });
        },
        ...
        show_confirm_move_menu_item: function(feed_id, $feed) {
            ...
            if (isFeed) {
                ...
                folder_view = PYTUNE.assets.folders.get_view($feed) ||
                              this.active_folder.folder_view;
        ...
        make_folders_multiselect: function(feed, in_folders) {
            var folders = PYTUNE.assets.get_folders();
        ...
        post_add_folder_to_folder: function(e, data) {
            if (data.folders) {
                PYTUNE.assets.folders.reset(_.compact(data.folders), {parse: true});
            }
            ...
            PYTUNE.assets.feeds.trigger('reset');
        ...
        manage_menu_mute_feed: function(feed_id, unmute) {
            var approve_list = _.pluck(PYTUNE.assets.feeds.filter(function(feed) {
                if (unmute) {
        ...
        hide_confirm_story_share_menu_item: function(shared) {
            ...
            var text = 'Share to your Pytuneblog';
            if (shared) {
                text = 'Shared';
        ...
        load_intelligence_slider: function() {
            ...
            if (unread_view == 0 && !PYTUNE.assets.preference('hide_read_feeds')) {
                unread_view = -1;
            }
        ...
        toggle_focus_in_slider: function() {
            ...
            var unread_view = this.get_unread_view_name();
            var all_mode = !PYTUNE.assets.preference('hide_read_feeds');
            var starred_mode = this.flags['feed_list_showing_starred'];
            if (!PYTUNE.assets.feeds.size()) return;
            
            var view_not_empty;
            if (unread_view == 'starred') {
                view_not_empty = PYTUNE.assets.starred_feeds.any(function(feed) { 
                    return feed.get('count');
                });
            } else if (unread_view == 'positive') {
                view_not_empty = PYTUNE.assets.feeds.any(function(feed) { 
                    return feed.get('ps');
                }) || PYTUNE.assets.social_feeds.any(function(feed) { 
                    return feed.get('ps');
                });
            } else {
                view_not_empty = PYTUNE.assets.feeds.any(function(feed) { 
            ...
            //     if (PYTUNE.assets.preference('unread_view') > 0) {
            //         this.slide_intelligence_slider(0);
            //     }
            // } else {
        ...
        slide_intelligence_slider: function(value, initial_load) {
            ...
            if (value <= -1) {
                value = 0;
                if (!initial_load) {
                    PYTUNE.assets.preference('hide_read_feeds', 0);
                }
                PYTUNE.assets.preference('unread_view', 0);
            } else if (value == 0) {
                if (!initial_load) {
                    PYTUNE.assets.preference('hide_read_feeds', 1);
                }
                PYTUNE.assets.preference('unread_view', 0);
            } else if (value >= 2) {
                if (!initial_load) {
                ...
                PYTUNE.assets.preference('unread_view', 2);
            } else if (value > 0) {
                if (!initial_load) {
                    PYTUNE.assets.preference('hide_read_feeds', 1);
                }
                PYTUNE.assets.preference('unread_view', 1);
            }
            this.flags['unread_threshold_temporarily'] = null;
            this.switch_feed_view_unread_view(value);
            if (PYTUNE.app.story_titles_header) {
                PYTUNE.app.story_titles_header.show_feed_hidden_story_title_indicator(true);
            }
            this.show_story_titles_above_intelligence_level({'animate': true, 'follow': true});
            ...
            PYTUNE.app.sidebar_header.toggle_hide_read_preference();
            PYTUNE.app.sidebar_header.count();
            PYTUNE.assets.folders.update_all_folder_visibility();
            PYTUNE.app.feed_list.scroll_to_selected();
        ...
        toggle_read_filter: function() {
            var read_filter = PYTUNE.assets.view_setting(this.active_feed, 'read_filter');
            var setting = {
                'read_filter': (read_filter == 'unread' ? 'all' : 'unread')
            };
            var changed = PYTUNE.assets.view_setting(this.active_feed, setting);
            if (!changed) return;
        ...
        switch_feed_view_unread_view: function(unread_view) {
            ...
            PYTUNE.assets.stories.each(function(story){ 
                story.unset('visible'); 
            });
        },
        ...
        get_unread_count: function(feed_id) {
            ...
            if (_.contains(['starred', 'read'], feed_id)) {
                ...
                return PYTUNE.assets.social_feeds.unread_counts();
            }
        ...
        show_story_titles_above_intelligence_level: function(opts) {
            ...
            PYTUNE.assets.stories.trigger('render:intelligence', options);
            
            if (!PYTUNE.assets.preference('feed_view_single_story')) {
                _.delay(function() {
                    PYTUNE.app.story_list.reset_story_positions();
                }, 500);
            }

            PYTUNE.app.story_list.show_correct_explainer();

            // PYTUNE.log(['Showing correct stories', this.story_view, unread_view_name, $stories_show.length, $stories_hide.length]);
            if (_.contains(['split', 'list', 'grid'],
                PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout'))) {
                PYTUNE.app.story_titles.fill_out();
            } else {
            ...
            if (options.follow && this.active_story) {
                PYTUNE.app.story_list.scroll_to_selected_story(self.active_story);
                PYTUNE.app.story_titles.scroll_to_selected_story(self.active_story);
            }
        },
        ...
        setup_socket_realtime_unread_counts: function(force) {
            if (!force && PYTUNE.Globals.is_anonymous) return;
            // if (!force && !PYTUNE.Globals.is_premium) return;
            if (this.socket && !this.socket.socket.connected) {
                ...
                var local = PYTUNE.Globals.debug || _.any([], function(hostname) {
                ...
                this.socket.on('connect', _.bind(function() {
                    ...
                    // PYTUNE.log(["Connected to real-time pubsub with " + active_feeds.length + " feeds."]);
                ...
                this.socket.on('feed:update', _.bind(function(feed_id, message) {
                    PYTUNE.log(['Real-time feed update', feed_id, message]);
                    this.feed_unread_count(feed_id, {realtime: true});
                }, this));
                
                this.socket.removeAllListeners(PYTUNE.Globals.username);
                this.socket.on('user:update', _.bind(function(username, message) {
                    ...
                    if (_.string.startsWith(message, 'feed:')) {
                        ...
                        if (PYTUNE.app.active_search) {
                            PYTUNE.app.active_search.update_indexing_progress(message);
                        }
                        ...
                        if (!PYTUNE.reader.flags['reloading_feeds']) {
                            console.log(["Reloading feeds due to server reload", PYTUNE.reader.flags['reloading_feeds']]);
                            PYTUNE.assets.load_feeds();
                        }
                    }
                }, this));
                ...
                this.socket.on('disconnect', _.bind(function() {
                    PYTUNE.log(["Lost connection to real-time pubsub. Falling back to polling."]);
                ...
                this.socket.on('error', _.bind(function() {
                    PYTUNE.log(["Can't connect to real-time pubsub."]);
                    this.flags.feed_refreshing_in_realtime = false;
                    this.setup_feed_refresh();
                    // $('.NB-module-content-account-realtime-subtitle').html($.make('b', 'Updating every 60 sec'));
                    $('.NB-module-content-account-realtime').attr('title', 'Updating sites every ' + this.flags.refresh_interval + ' seconds...').addClass('NB-error').removeClass('NB-active');
        ...
        send_socket_active_feeds: function() {
            ...
            if (active_feeds.length) {
                this.socket.emit('subscribe:feeds', active_feeds, PYTUNE.Globals.username);
            }
        ...
        setup_feed_refresh: function(new_feeds) {
            ...
            PYTUNE.log(["Setting refresh interval to every " + this.flags.refresh_interval + " seconds."]);
        ...
        force_feed_refresh: function(feed_id, new_feed_id) {
            ...
            }, true, new_feed_id, PYTUNE.app.taskbar_info.show_stories_error);
        },
        ...
        post_feed_refresh: function(data) {
            ...
            PYTUNE.app.sidebar_header.update_interactions_count(data.interactions_count);
        ...
        show_mouse_indicator: function() {
            ...
            if (PYTUNE.assets.preference('feed_view_single_story')) return;
        ...
        position_mouse_indicator: function() {
            if (!_.contains(['split', 'full'], 
                PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout'))) return;
            ...
            // console.log(["position_mouse_indicator", PYTUNE.reader.cache.mouse_position_y, position]);
        ...
        post_google_reader_connect: function(data) {
            if (PYTUNE.intro) {
                PYTUNE.intro.start_import_from_google_reader(data);
            } else {
                this.start_import_from_google_reader();
            }
        },
        ...
        finish_import_from_google_reader: function(e, data) {
            ...
            if (data.code >= 1) {
                ...
                $('.NB-progress-link', $progress).html($.make('a', { 
                    ...
                    href: PYTUNE.URLs['google-reader-authorize']
                }, ['Try importing again']));
        ...
        setup_dashboard_graphs: function() {
            if (PYTUNE.Globals.debug) return;
            
            // Reload dashboard graphs every 10 minutes.
            var reload_interval = PYTUNE.Globals.is_staff ? 60*1000 : 10*60*1000;
        ...
        setup_feedback_table: function() {
            ...
            var reload_interval = PYTUNE.Globals.is_staff ? 30*1000 : 5*60*1000;
        ...
        setup_unfetched_feed_check: function() {
            this.locks.unfetched_feed_check = setInterval(_.bind(function() {
                var unfetched_feeds = PYTUNE.assets.unfetched_feeds();
        ...
        handle_clicks: function(elem, e) {
            ...
            $.targetIs(e, { tagSelector: '.NB-menu-manage-feed-stats' }, function($t, $p){
                ...
                if (!$t.hasClass('NB-disabled')) {
                    var feed_id = $t.parents('.NB-menu-manage').data('feed_id');
                    PYTUNE.log(['statistics feed_id', feed_id]);
            ...
            $.targetIs(e, { tagSelector: '.NB-menu-manage-import' }, function($t, $p){
                ...
                if (!$t.hasClass('NB-disabled')) {
                    $.modal.close(function() {
                        PYTUNE.reader.open_intro_modal({
                            'page_number': 2,
                            'force_import': true
                        });
                    });
                ...
                if (!$t.hasClass('NB-disabled')) {
                    $.modal.close(function() {
                        self.open_feedchooser_modal({
                            'chooser_only': PYTUNE.Globals.is_premium
                        });
                    });
                }
            });  
            ...
            $.targetIs(e, { tagSelector: '.NB-menu-manage-story-read' }, function($t, $p){
                ...
                var story_id = $t.closest('.NB-menu-manage').data('story_id');
                var story = self.model.get_story(story_id);
                PYTUNE.assets.stories.mark_read(story);
            });  
            ...
            $.targetIs(e, { tagSelector: '.NB-recommended-try' }, function($t, $p){
                ...
                var feed_id = $t.closest('.NB-recommended').data('feed-id');
                self.open_feed(feed_id, {'feed': new PYTUNE.Models.Feed({
            ...
            $.targetIs(e, { tagSelector: '.NB-tryout-signup' }, function($t, $p){
                ...
                if (PYTUNE.welcome) {
                    PYTUNE.welcome.show_signin_form();
                }
            }); 
            ...
            $.targetIs(e, { tagSelector: '.NB-splash-meta-about' }, function($t, $p){
              e.preventDefault();
              PYTUNE.about = new PYTUNE.About();
            }); 
            $.targetIs(e, { tagSelector: '.NB-splash-meta-faq' }, function($t, $p){
              e.preventDefault();
              PYTUNE.faq = new PYTUNE.Faq();
            }); 
        ...
        handle_keystrokes: function() {      
            ...
            $document.bind('keydown', 'down', function(e) {
                e.preventDefault();
                if (PYTUNE.assets.preference('keyboard_verticalarrows') == 'scroll') {
                    var amount = PYTUNE.assets.preference('arrow_scroll_spacing');
            ...
            $document.bind('keydown', 'left', function(e) {
                e.preventDefault();
                if (PYTUNE.assets.preference('keyboard_horizontalarrows') == 'site') {
                    self.show_next_feed(-1);
                } else {
            ...
            $document.bind('keydown', 'enter', function(e) {
                ...
                if (self.flags['feed_view_showing_story_view']) {
                    ...
                    PYTUNE.app.story_tab_view.prepare_story(null, true);
                    PYTUNE.app.story_tab_view.open_story();
                }
            });
            ...
            $document.bind('keydown', 'space', function(e) {
                e.preventDefault();
                var amount = PYTUNE.assets.preference('space_scroll_spacing');
            ...
            $document.bind('keydown', 'x', function(e) {
                e.preventDefault();
                var story = PYTUNE.reader.active_story;
                if (story && story.get('selected')) {
                    PYTUNE.reader.active_story.story_title_view.collapse_story();
                } else if (story && !story.get('selected')) {
                    PYTUNE.reader.active_story.set('selected', true);
                }
            });
            $document.bind('keydown', 'shift+x', function(e) {
                e.preventDefault();
                PYTUNE.reader.active_story.story_view.expand_story();
            });
            $document.bind('keydown', 'shift+y', function(e) {
                e.preventDefault();
                if (!PYTUNE.reader.active_story) return;
                var story = PYTUNE.assets.get_story(PYTUNE.reader.active_story);
                var timestamp = story.get('story_timestamp');
            ...
            $document.bind('keydown', 'g', function(e) {
                e.preventDefault();
                PYTUNE.app.feed_selector.toggle();
            });
            $document.bind('keydown', '/', function(e) {
                e.preventDefault();
                if (PYTUNE.app.story_titles_header.search_view) {
                    PYTUNE.app.story_titles_header.search_view.focus();
                }
            });
