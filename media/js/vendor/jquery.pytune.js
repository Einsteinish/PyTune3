if (typeof PYTUNE.Globals == 'undefined') PYTUNE.Globals = {};

/* ============================= */
/* = Core PyTune Javascript = */
/* ============================= */
...
(function($) {
    ...
    $.fn.extend({
        ...
        isScrollVisible: function($elem, partial) {
            ...
            // PYTUNE.log(['isScrollVisible', docViewTop, docViewBottom, elemTop, elemBottom]);
    ...
    $.extend({
        ...
        favicon: function(feed, empty_on_missing) {
            if (_.isNumber(feed)) return PYTUNE.URLs.favicon.replace('{id}', feed);
            ...
            else if (_.string.include(feed.id, 'social:')) return PYTUNE.Globals.MEDIA_URL + 'img/reader/default_profile_photo.png';
            else if (empty_on_missing) return 'data:image/png;base64,R0lGODlhAQABAIAAAAAAAAAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==';
            else if (_.isNumber(feed.id)) return PYTUNE.URLs.favicon.replace('{id}', feed.id);
            else if (feed.get('favicon_url')) return feed.get('favicon_url');
            else if (feed.is_starred()) return PYTUNE.Globals.MEDIA_URL + '/img/reader/tag.png';
            return PYTUNE.Globals.MEDIA_URL + '/img/icons/circular/world.png';
        },
        ...
        targetIs: function(e, opts, callback){
            ...
            if(fails){
                ...
                if(callback && typeof callback == 'function'){
                    // PYTUNE.log(['Target', e, opts.tagSelector, opts]);
                    callback($t, $p);
                }
                return true;
            }
        ...
        closest: function(value, array) {
            ...
            for (var i in array) {
                ...
                if (next_value <= offset && Math.abs(next_value) < closest) {
                    ...
                    // PYTUNE.log(['Not Closest', index, next_value, value, closest]);
                    return index;
                }
            }
            return index;
