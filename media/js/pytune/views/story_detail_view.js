PYTUNE.Views.StoryDetailView = Backbone.View.extend({
    ...
    render: function() {
        ...
        this.sideoptions_view = new PYTUNE.Views.StorySideoptionsView({
            model: this.model, 
            el: this.el
        });
        ...
        params['story_share_view'] = this.sideoptions_view.share_view.template({
            story: this.model,
            social_services: PYTUNE.assets.social_services,
            profile: PYTUNE.assets.user_profile
        });
    ...
    get_render_params: function() {
        this.feed = PYTUNE.assets.get_feed(this.model.get('story_feed_id'));
        this.classifiers = PYTUNE.assets.classifiers[this.model.get('story_feed_id')];
        var show_feed_title = PYTUNE.reader.flags.river_view || 
                              PYTUNE.reader.flags.social_view || 
    ...
    generate_gradients: function() {
        ...
        $header.css('background-image', PYTUNE.utils.generate_gradient(this.feed, 'webkit'));
        $header.css('background-image', PYTUNE.utils.generate_gradient(this.feed, 'moz'));
        $header.css('borderTop',        PYTUNE.utils.generate_gradient(this.feed, 'border'));
        $header.css('borderBottom',     PYTUNE.utils.generate_gradient(this.feed, 'border'));
        $header.css('textShadow',       PYTUNE.utils.generate_shadow(this.feed));
    },
    
    is_truncatable: function() {
        return PYTUNE.assets.preference("truncate_story") == 'all' || 
               (PYTUNE.assets.preference("truncate_story") == 'social' &&
                PYTUNE.reader.flags['social_view']);
    },
    ...
    make_story_title: function(story) {
        ...
        var classifiers = PYTUNE.assets.classifiers[story.get('story_feed_id')];
    ...
    render_comments: function() {
        ...
        if (this.model.get("comment_count") || this.model.get("share_count")) {
            var comments_view = new PYTUNE.Views.StoryCommentsView({model: this.model});
    ...
    render_intelligence: function(options) {
        ...
        var unread_view = PYTUNE.reader.get_unread_view_score();
    ...
    toggle_classes: function() {
        ...
        if (this.model.changedAttributes()) {
            // PYTUNE.log(["Story changed", this.model.changedAttributes(), this.model.previousAttributes()]);
        }
    ...
    setup_classes: function() {
        ...
        this.$el.toggleClass('NB-river-story', PYTUNE.reader.flags.river_view ||
                                               PYTUNE.reader.flags.social_view);
        ...
        if (PYTUNE.assets.preference('show_tooltips')) {
    ...
    toggle_selected: function(model, selected, options) {
        ...
        if (selected && options.scroll_to_comments) {
        ...
        } else if (PYTUNE.assets.preference('feed_view_single_story')) {
            PYTUNE.app.story_list.scroll_to_selected_story(model, {
                'scroll_to_top': true
            });
        } else if (selected && 
            ...
            PYTUNE.app.story_list.scroll_to_selected_story(model, options);
        }
        
        if (PYTUNE.reader.flags['feed_view_showing_story_view'] ||
            PYTUNE.reader.flags['temporary_story_view']) {
    ...
    expand_story: function(options) {
        ...
        this._fetch_interval = setInterval(function() {
            PYTUNE.app.story_list.fetch_story_locations_in_feed_view();
        }, 250);
    ...
    attach_audio_handler: function() {
        _.delay(_.bind(function() {
            ...
            var audio_opts = window.a = {
                imageLocation: PYTUNE.Globals.MEDIA_URL + 'img/reader/player-graphics.gif',
                swfLocation: PYTUNE.Globals.MEDIA_URL + 'flash/audiojs.swf',
                preload: false
            };
    ...
    click_link_in_story: function(e) {
        if (PYTUNE.hotkeys.shift) return;
        ...
        if (_.string.contains(href, "#")) {
            ...
            if ($footnote.length) {
                ...
                if (_.contains(['list', 'grid'], PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout'))) {
                    $scroll = PYTUNE.reader.$s.$story_titles;
                } else if (PYTUNE.reader.flags['temporary_story_view'] || 
                    PYTUNE.reader.story_view == 'text') {
                    $scroll = PYTUNE.reader.$s.$text_view;
                } else {
                    $scroll = PYTUNE.reader.$s.$feed_scroll;
                }
        ...
        if (PYTUNE.assets.preference('new_window') == 1) {
    ...
    show_manage_menu_rightclick: function(e) {
        if (!PYTUNE.assets.preference('show_contextmenus')) return;
    ...
    show_manage_menu: function(e) {
        e.preventDefault();
        e.stopPropagation();
        PYTUNE.reader.show_manage_menu('story', this.$el, {
    ...
    show_story_changes: function() {
        PYTUNE.assets.fetch_story_changes(this.model.get('story_hash'), !this.model.get('showing_diff'), _.bind(function(data) {
    ...
    open_feed: function() {
        PYTUNE.reader.open_feed(this.model.get('story_feed_id'));
    },
    ...
    save_classifier: function(e) {
        ...
        PYTUNE.assets.classifiers[feed_id][classifier_type+'s'][value] = score;
        PYTUNE.assets.recalculate_story_scores(feed_id, {story_view: this});
        PYTUNE.assets.save_classifier(data, function(resp) {
            PYTUNE.reader.feed_unread_count(feed_id);
        });
    ...
    open_story_trainer: function() {
        ...
        PYTUNE.reader.open_story_trainer(this.model.id, feed_id, options);
    },
    ...
    scroll_to_comments: function() {
        if (_.contains(['list', 'grid'], PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout'))) {
            PYTUNE.app.story_titles.scroll_to_selected_story(this.model, {
                scroll_to_comments: true,
                scroll_offset: -50
            });
        } else {
            PYTUNE.app.story_list.scroll_to_selected_story(this.model, {
                scroll_to_comments: true,
                scroll_offset: -50
            });
        }
