PYTUNE.ReaderSocialProfile = function(user_id, options) {
    var defaults = {
        width: 800
    };
    ...
    this.profiles = new PYTUNE.Collections.Users();
    user_id = parseInt(_.string.ltrim(user_id, 'social:'), 10);
    this.runner(user_id);
};

PYTUNE.ReaderSocialProfile.prototype = new PYTUNE.Modal;

_.extend(PYTUNE.ReaderSocialProfile.prototype, {
    ...
    make_modal: function() {
        var self = this;
        this.$profile = new PYTUNE.Views.SocialProfileBadge({
            model: this.profile,
            embiggen: true,
            photo_size: 'large',
    ...
    populate_friends: function() {
        // PYTUNE.log(["populate_friends", this.profile.get('followers_youknow')]);
    ...
    make_profile_badges: function(user_ids, profiles) {
        ...
        var $badges = $.make('div', { className: 'NB-profile-links' }, _.map(user_ids, function(user_id) {
            var user = new PYTUNE.Models.User(profiles[user_id]);
