PYTUNE.Views.StoryTitleView = Backbone.View.extend({
    ...
    render: function() {
        ...
        this.$el.html(this[template_name]({
            story    : this.model,
            feed     : (PYTUNE.reader.flags.river_view || PYTUNE.reader.flags.social_view) &&
                        PYTUNE.assets.get_feed(this.model.get('story_feed_id')),
            ...
            <% if (story.get("comment_count_friends")) { %>\
                <div class="NB-storytitles-shares">\
                    <% _.each(story.get("commented_by_friends"), function(user_id) { %>\
                        <img class="NB-user-avatar" src="<%= PYTUNE.assets.user_profiles.find(user_id).get("photo_url") %>">\
                    <% }) %>\
                </div>\
            <% } %>\
    ...
    render_inline_story_detail: function(temporary_text) {
        if (PYTUNE.reader.story_view == 'text' || temporary_text) {
            this.text_view = new PYTUNE.Views.TextTabView({
                el: null,
            ...
            this.story_detail = new PYTUNE.Views.StoryDetailView({
                model: this.model,
    ...
    show_content_preview: function() {
        var preference = PYTUNE.assets.preference('show_content_preview');
    ...
    show_image_preview: function() {
        if (!PYTUNE.assets.preference('show_image_preview')) {
            return false;
        }

        var story_layout = PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout');
        var pane_anchor = PYTUNE.assets.preference('story_pane_anchor');
    ...
    color_feedbar: function() {
        ...
        var feed = PYTUNE.assets.get_feed(this.model.get('story_feed_id'));
        if (!feed) return;
    ...
    load_youtube_embeds: function() {
        ...
        if (i) {
            this.$(".NB-storytitles-story-image").css({
                'display': 'block',
                'background-image': "url("+PYTUNE.Globals.MEDIA_URL+"img/reader/youtube_play.png), url(" + "http://img.youtube.com/vi/" + i + "/0.jpg" + ")"
            });
            return true;
        }
    },
    ...
    toggle_read_status: function(model, read_status, options) {
        ...
        if (options.error_marking_unread) {
            var pane_alignment = PYTUNE.assets.preference('story_pane_anchor');
    ...
    toggle_selected: function(model, selected, options) {
        ...
        if (!!this.model.get('selected')) {
            ...
            // PYTUNE.app.story_titles.scroll_to_selected_story(this.model, options);
        } else {
            this.destroy_inline_story_detail();
        }
    },
    ...
    toggle_starred: function() {
        var story_titles_visible = _.contains(['split', 'full'], PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout'));
        var pane_alignment = PYTUNE.assets.preference('story_pane_anchor');
    ...
    select_story_shared: function(e) {
        ...
        if (PYTUNE.reader.story_view == 'page') {
            PYTUNE.reader.switch_taskbar_view('feed', {skip_save_type: 'page'});
        }
    ...
    show_manage_menu: function(e) {
        e.preventDefault();
        e.stopPropagation();
        // PYTUNE.log(["showing manage menu", this.model.is_social() ? 'socialfeed' : 'feed', $(this.el), this]);
        PYTUNE.reader.show_manage_menu('story', this.$st, {
    ...
    open_story_in_story_view: function(e) {
        e.preventDefault();
        e.stopPropagation();
        PYTUNE.app.story_tab_view.prepare_story(this.model, true);
        PYTUNE.app.story_tab_view.open_story(this.model);
        return false;
    }
