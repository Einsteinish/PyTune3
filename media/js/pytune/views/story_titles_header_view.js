PYTUNE.Views.StoryTitlesHeader = Backbone.View.extend({
    ...
    render: function(options) {
        ...
        this.showing_fake_folder = PYTUNE.reader.flags['river_view'] && 
            PYTUNE.reader.active_folder && 
            (PYTUNE.reader.active_folder.get('fake') || !PYTUNE.reader.active_folder.get('folder_title'));
        ...
        if (PYTUNE.reader.flags['starred_view']) {
                ...
                <div class="NB-folder NB-no-hover">\
                    ...
                    <div class="NB-feedbar-options-container">\
                        <span class="NB-feedbar-options">\
                            ...
                            <%= PYTUNE.assets.view_setting("starred", "order") %>\
                        </span>\
                    </div>\
            ...
            ', {
                tag: PYTUNE.reader.flags['starred_tag']
            }));
            this.search_view = new PYTUNE.Views.FeedSearchView({
                feedbar_view: this
            }).render();
            this.search_view.blur_search();
            $(".NB-search-container", $view).html(this.search_view.$el);
        } else if (PYTUNE.reader.active_feed == "read") {
            $view = $(_.template('\
                <div class="NB-folder NB-no-hover">\
                    <div class="NB-feedbar-options-container">\
                        <span class="NB-feedbar-options">\
                            <div class="NB-icon"></div>\
                            <%= PYTUNE.assets.view_setting("read", "order") %>\
                        </span>\
                    </div>\
                ...
                <div class="NB-folder NB-no-hover NB-folder-<%= all_stories ? "river" : "fake" %>">\
                    ...
                    <% if (show_options) { %>\
                        <div class="NB-feedbar-options-container">\
                            <span class="NB-feedbar-options">\
                                <div class="NB-icon"></div>\
                                <%= PYTUNE.assets.view_setting(folder_id, "read_filter") %>\
                                &middot;\
                                <%= PYTUNE.assets.view_setting(folder_id, "order") %>\
                            </span>\
                        </div>\
                    <% } %>\
            ...
            ', {
                ...
                folder_id: PYTUNE.reader.active_feed,
                all_stories: PYTUNE.reader.active_feed == "river:",
                show_options: !PYTUNE.reader.active_folder.get('fake') ||
                              PYTUNE.reader.active_folder.get('show_options')
            }));
            this.search_view = new PYTUNE.Views.FeedSearchView({
                feedbar_view: this
            }).render();
            this.search_view.blur_search();
            $(".NB-search-container", $view).html(this.search_view.$el);
        } else if (PYTUNE.reader.flags['river_view'] && 
                   PYTUNE.reader.active_folder &&
                   PYTUNE.reader.active_folder.get('folder_title')) {
            this.view = new PYTUNE.Views.Folder({
                model: PYTUNE.reader.active_folder,
                collection: PYTUNE.reader.active_folder.folder_view.collection,
                feedbar: true,
                only_title: true
            }).render();    
            $view = this.view.$el;    
            ...
            this.view = new PYTUNE.Views.FeedTitleView({
                model: PYTUNE.assets.get_feed(this.options.feed_id), 
                type: 'story'
            }).render();
            $view = this.view.$el;
            this.search_view = this.view.search_view;
        ...
        if (PYTUNE.reader.flags.searching) {
            this.focus_search();
        }
    ...
    fake_folder_title: function() {
        ...
        if (PYTUNE.reader.flags['social_view']) {
            if (PYTUNE.reader.flags['global_pytuneblogs']) {
    ...
    show_feed_hidden_story_title_indicator: function(is_feed_load) {
        ...
        if (PYTUNE.reader.flags.search) return;
        if (PYTUNE.reader.flags['feed_list_showing_starred']) return;
        PYTUNE.reader.flags['unread_threshold_temporarily'] = null;
        
        var unread_view_name = PYTUNE.reader.get_unread_view_name();
        var $indicator = this.$('.NB-story-title-indicator');
        var unread_hidden_stories;
        if (PYTUNE.reader.flags['river_view']) {
            unread_hidden_stories = PYTUNE.reader.active_folder &&
                                    PYTUNE.reader.active_folder.folders &&
                                    PYTUNE.reader.active_folder.folders.unread_counts &&
                                    PYTUNE.reader.active_folder.folders.unread_counts().ng;
        } else {
            unread_hidden_stories = PYTUNE.assets.active_feed.unread_counts().ng;
        }
        var hidden_stories = unread_hidden_stories || !!PYTUNE.assets.stories.hidden().length;
        if (!hidden_stories) {
            $indicator.hide();
            return;
        }
    ...
    show_hidden_story_titles: function() {
        var $indicator = this.$('.NB-story-title-indicator');
        var temp_unread_view_name = PYTUNE.reader.get_unread_view_name();
        var unread_view_name = PYTUNE.reader.get_unread_view_name(null, true);
        var hidden_stories_at_threshold = PYTUNE.assets.stories.any(function(story) {
        ...
        var hidden_stories_below_threshold = temp_unread_view_name == 'positive' && 
                                             PYTUNE.assets.stories.any(function(story) {
            return story.score() < 0;
        });
        
        PYTUNE.log(['show_hidden_story_titles', hidden_stories_at_threshold, hidden_stories_below_threshold, unread_view_name, temp_unread_view_name, PYTUNE.reader.flags['unread_threshold_temporarily']]);
        ...
        if (temp_unread_view_name == 'positive' && 
            ...
            PYTUNE.reader.flags['unread_threshold_temporarily'] = 'neutral';
            PYTUNE.reader.show_story_titles_above_intelligence_level({
        ...
        } else if (PYTUNE.reader.flags['unread_threshold_temporarily'] != 'negative') {
            PYTUNE.reader.flags['unread_threshold_temporarily'] = 'negative';
            PYTUNE.reader.show_story_titles_above_intelligence_level({
    ...
    open_options_popover: function(e) {
        if (!(this.showing_fake_folder ||
              PYTUNE.reader.active_feed == "read" ||
              PYTUNE.reader.flags['starred_view'])) return;
        
        PYTUNE.FeedOptionsPopover.create({
            anchor: this.$(".NB-feedbar-options"),
            feed_id: PYTUNE.reader.active_feed
        });
    },
    
    mark_folder_as_read: function(e, days_back) {
        if (!this.showing_fake_folder) return;
        if (PYTUNE.assets.preference('mark_read_river_confirm')) {
            PYTUNE.reader.open_mark_read_modal({days: days_back || 0});
        } else {
            PYTUNE.reader.mark_folder_as_read();
        }
    ...
    expand_mark_read: function() {
        if (!this.showing_fake_folder) return;
        PYTUNE.Views.FeedTitleView.prototype.expand_mark_read.call(this);
    }
