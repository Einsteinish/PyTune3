PYTUNE.ReaderProfileEditor = function(options) {
    var defaults = {
        width: 800
    };
...
PYTUNE.ReaderProfileEditor.prototype = new PYTUNE.Modal;

_.extend(PYTUNE.ReaderProfileEditor.prototype, {
    ...
    make_modal: function() {
        ...
        this.$modal = $.make('div', { className: 'NB-modal NB-modal-profile-editor' }, [
            $.make('div', { className: 'NB-modal-tabs' }, [
                ...
                $.make('div', { className: 'NB-modal-tab NB-modal-tab-pytuneblog' }, 'Pytuneblog')
            ]),
                    ...
                    $.make('div', { className: 'NB-modal-section NB-friends-profile'}, [
                        $.make('form', [
                            ...
                            $.make('label', { 'for': 'NB-profile-privacy-public' }, [
                                'Privacy',
                                (!PYTUNE.Globals.is_premium && $.make('div', { className: 'NB-profile-privacy-notpremium' }, [
                                    'You must have a ',
            ...
            $.make('div', { className: 'NB-tab NB-tab-pytuneblog' }, [
                $.make('fieldset', [
                    $.make('legend', 'Your Pytuneblog'),
                    $.make('div', { className: 'NB-modal-section NB-profile-editor-pytuneblog-preview' }, [
                        $.make('label', { 'for': 'NB-profile-pytuneblog-address' }, 'Pytuneblog address'),
                        $.make('a', { href: this.profile.get('feed_link'), target: '_blank', className: 'NB-profile-pytuneblog-address NB-splash-link' }, this.profile.get('feed_link')),
                        $.make('label', { 'for': 'NB-profile-pytuneblog-title' }, 'Pytuneblog title'),
                        $.make('input', { type: 'text', id: 'NB-profile-pytuneblog-title', name: 'pytuneblog_title', value: this.profile.get('feed_title'), className: 'NB-input NB-profile-pytuneblog-title' }),
                        $.make('label', 'Background color'),
                        this.make_color_palette()
                    ])
                ]),
                $.make('fieldset', [
                    $.make('legend', 'Custom CSS for your Pytuneblog'),
                    $.make('div', { className: 'NB-modal-section NB-profile-editor-pytuneblog-custom-css'}, [
                        $.make('textarea', { 'className': 'NB-profile-pytuneblog-css', name: 'custom_css' }, this.profile.get('custom_css'))
                    ])
                ]),
                $.make('fieldset', [
                    $.make('legend', 'Pytuneblog Options'),
                            ...
                            $.make('div', { className: 'NB-preference-options' }, [
                                $.make('div', [
                                    ...
                                    $.make('label', { 'for': 'NB-preference-permalinkdirect-0' }, [
                                        'Link to my pytuneblog and shared comments'
                                    ])
                                ]),
                                $.make('div', [
                ...
                $.make('div', { className: 'NB-disabled NB-modal-submit-green NB-pytuneblog-save-button NB-modal-submit-button' }, 'Change your pytuneblog settings above')
            ]),
    ...
    make_color_palette: function() {
        ...
        var $colors = $.make('div', { className: 'NB-profile-pytuneblog-colors' });
        _.each(colors, function(color_line) {
            var $color_line = $.make('div', { className: 'NB-profile-pytuneblog-colorline' });
            _.each(color_line, function(color) {
                var $color = $.make('span', { className: 'NB-profile-pytuneblog-color', style: 'background-color: ' + color }).data('color', color);
    ...
    choose_color: function() {
        ...
        var $colors = $('.NB-profile-pytuneblog-color', this.$modal);
    ...
    make_profile_section: function() {
        ...
        $profile_badge = new PYTUNE.Views.SocialProfileBadge({model: profile});
                ...
                $.make('div', { className: 'NB-friends-photo-image' }, [
                    $.make('label', { 'for': 'NB-profile-photo-service-'+service }, [
                        ...
                        $.make('img', { src: service == 'nothing' || !this.services[service][service+'_picture_url'] ?
                            PYTUNE.Globals.MEDIA_URL + 'img/reader/default_profile_photo.png' :
    ...
    save_pytuneblog: function() {
        var data = this.serialize_preferences($(".NB-tab-pytuneblog"));
        data['custom_bgcolor'] = $('.NB-profile-pytuneblog-color.NB-active', this.$modal).data('color');
        this.model.save_pytuneblog_settings(data, _.bind(function() {
            this.disable_save_pytuneblog();
        }, this));
        this.disable_save_pytuneblog();
        $('.NB-pytuneblog-save-button', this.$modal).text('Saving...');
    },
    ...
    set_active_color: function($color) {
        $('.NB-profile-pytuneblog-color.NB-active', this.$modal).removeClass('NB-active');
        $color.addClass('NB-active');
        this.enable_save_pytuneblog();
    },
    ...
    handle_click: function(elem, e) {
        ...
        $.targetIs(e, { tagSelector: '.NB-modal-tab' }, function($t, $p) {
            ...
            if ($t.hasClass('NB-modal-tab-profile')) {
                newtab = 'profile';
            } else if ($t.hasClass('NB-modal-tab-pytuneblog')) {
                newtab = 'pytuneblog';
            }
        ...
        $.targetIs(e, { tagSelector: '.NB-pytuneblog-save-button' }, function($t, $p) {
            e.preventDefault();
            
            self.save_pytuneblog();
        });
        ...
        $.targetIs(e, { tagSelector: '.NB-profile-pytuneblog-color' }, function($t, $p) {
            e.preventDefault();
        ...
        var params = {
            ...
            success: _.bind(function(data, status) {
                if (data.code < 0) {
                    ...
                    PYTUNE.assets.user_profile.set(data.user_profile);
                    this.services = data.services;
                    this.make_profile_section();
                    this.make_profile_photo_chooser();
                }
    ...
    delegate_change: function() {
        ...
        $('.NB-tab-pytuneblog', this.$modal).delegate('input[type=text],textarea', 'keydown', _.bind(this.enable_save_pytuneblog, this));
        $('.NB-tab-pytuneblog', this.$modal).delegate('input,textarea', 'change', _.bind(this.enable_save_pytuneblog, this));
    },
    ...
    enable_save_pytuneblog: function() {
        $('.NB-pytuneblog-save-button', this.$modal)
            .removeClass('NB-disabled')
            .text('Save My Pytuneblog Settings');
    },
    ...
    disable_save_pytuneblog: function() {
        $('.NB-pytuneblog-save-button', this.$modal)
            .addClass('NB-disabled')
            .text('Saved!');
    }
