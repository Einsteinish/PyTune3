PYTUNE.Views.UnreadCount = Backbone.View.extend({
    ...
    initialize: function() {
        ...
        if (!this.options.stale && !this.options.feed_chooser) {
            if (this.model) {
                var starred_feed = PYTUNE.assets.starred_feeds.get_feed(this.model.id);
                if (starred_feed) {
    ...
    flash: function() {
        ...
        if (!PYTUNE.assets.preference('animations')) return;
