PYTUNE.AssetModel = Backbone.Router.extend({
    ...
    initialize: function() {
        ...
        this.feeds = new PYTUNE.Collections.Feeds();
        this.social_feeds = new PYTUNE.Collections.SocialSubscriptions();
        this.folders = new PYTUNE.Collections.Folders([]);
        this.favicons = {};
        this.stories = new PYTUNE.Collections.Stories();
        this.starred_feeds = new PYTUNE.Collections.StarredFeeds();
        this.queued_read_stories = {};
        this.classifiers = {};
        this.friends = {};
        this.profile = {};
        this.user_profile = new PYTUNE.Models.User();
        this.social_services = {};
        this.user_profiles = new PYTUNE.Collections.Users();
        this.follower_profiles = new PYTUNE.Collections.Users();
        this.following_profiles = new PYTUNE.Collections.Users();
    ...
    make_request: function(url, data, callback, error_callback, options) {
        ...
        this.ajax[options['ajax_group']].add(_.extend({
            ...
            cacheResponse: false,
            beforeSend: function() {
                // PYTUNE.log(['beforeSend', options]);
                $.isFunction(options['beforeSend']) && options['beforeSend']();
                return true;
            },
            success: function(o) {
                // PYTUNE.log(['make_request 1', o]);
            ...
            error: function(e, textStatus, errorThrown) {
                ...
                PYTUNE.log(['AJAX Error', e, e.status, textStatus, errorThrown, 
                              !!error_callback, error_callback, $.isFunction(callback)]);
                
                if (options.retry) {
                    PYTUNE.log(['Retrying...', url, data, !!callback, !!error_callback, options]);
                    options.retry = false;
                    self.make_request(url, data, callback, error_callback, options);
                    return;
                }
                if (errorThrown == "timeout") textStatus = "PyTune timed out trying<br />to connect. Just try again.";
                if (error_callback) {
    ...
    mark_story_as_read: function(story, feed, callback) {
        ...
        if (!story.get('read_status')) {
            ...
            if (PYTUNE.Globals.is_authenticated) {
                ...
                // PYTUNE.log(['Marking Read', this.queued_read_stories, story.id]);
            ...
            this.make_request('/reader/feed/'+feed_id,
                {
                    ...
                    query: PYTUNE.reader.flags.search,
                    include_hidden: true
                }, pre_callback,
                error_callback,
                {
    ...
    fetch_read_stories: function(page, callback, error_callback, first_load) {
        ...
        var pre_callback = function(data) {
            if (!PYTUNE.Globals.is_premium && PYTUNE.Globals.is_authenticated) {
                if (first_load) {
    ...
    fetch_river_pytuneblogs_stories: function(feed_id, page, options, callback, error_callback, first_load) {
        var self = this;
    ...
    refresh_feeds: function(callback, has_unfetched_feeds, feed_id, error_callback) {
        ...
        if (PYTUNE.Globals.is_authenticated || feed_id) {
    ...
    post_refresh_feeds: function(data, callback, options) {
        ...
        _.each(data.feeds, _.bind(function(feed, feed_id) {
            ...
            if (feed.id && feed_id != feed.id) {
                PYTUNE.log(['Dupe feed being refreshed', feed_id, feed.id, this.feeds.get(f), feed]);
    ...
    refresh_feed: function(feed_id, callback) {
        ...
        var pre_callback = function(data) {
            // PYTUNE.log(['refresh_feed pre_callback', data]);
            self.load_feed_precallback(data, feed_id, callback);
        };
        
        // PYTUNE.log(['refresh_feed', feed_id, page, first_load, callback, pre_callback]);
        if (feed_id) {
            this.make_request('/reader/feed/'+feed_id,
                {
                    page: 0,
    ...
    add_social_feed: function(feed) {
        ...
        if (!social_feed) {
            ...
            social_feed = new PYTUNE.Models.SocialSubscription(attributes);
    ...
    delete_feed_by_url: function(url, in_folder, callback) {
        this.make_request('/reader/delete_feed_by_url/', {
          ...
          callback({'message': PYTUNE.Globals.is_anonymous ? 'Please create an account. Not much to do without an account.' : 'There was a problem trying to add this site. Please try a different URL.'});
        });
    },
    ...
    save_add_url: function(url, folder, callback, options) {
        ...
        this.make_request('/reader/add_url/', {
          ...
          callback({'message': PYTUNE.Globals.is_anonymous ? 'Please create an account. Not much to do without an account.' : data.message || 'There was a problem trying to add this site. Please try a different URL.'});
        });
    },
    ...
    save_add_folder: function(folder, parent_folder, callback) {
        this.make_request('/reader/add_folder/', {
          ...
          callback({'message': PYTUNE.Globals.is_anonymous ? 'Please create an account. Not much to do without an account.' : data.message || 'There was a problem trying to add this folder.'});
        });
    },
    ...
    preference: function(preference, value, callback) {
        if (typeof value == 'undefined') {
            var pref = PYTUNE.Preferences[preference];
            if ((/^\d+$/).test(pref)) return parseInt(pref, 10);
            return pref;
        }
        
        if (PYTUNE.Preferences[preference] == value) {
          return $.isFunction(callback) && callback();
        }
        
        PYTUNE.Preferences[preference] = value;
        var preferences = {};
    ...
    view_setting: function(feed_id, setting, callback) {
        if (PYTUNE.reader.flags['feed_list_showing_starred'] && 
        ...
        if (_.isUndefined(setting) || _.isString(setting)) {
            ...
            var feed = PYTUNE.Preferences.view_settings[feed_id+''];
            var default_setting = PYTUNE.Preferences['default_' + setting];
            if (setting == 'layout') default_setting = PYTUNE.Preferences['story_layout'];
        ...
        var view_settings = _.clone(PYTUNE.Preferences.view_settings[feed_id+'']) || {};
        ...
        if (!_.isEqual(PYTUNE.Preferences.view_settings[feed_id+''], view_settings)) {
            PYTUNE.Preferences.view_settings[feed_id+''] = view_settings;
    ...
    clear_view_settings: function(view_setting_type, callback) {
        var pre_callback = _.bind(function(data) {
            if (data.view_settings) {
                PYTUNE.Preferences.view_settings = data.view_settings;
            }
            callback(data);
        }, this);
    ...
    collapsed_folders: function(folder_title, is_collapsed, callback) {
        var folders = PYTUNE.Preferences.collapsed_folders;
        var changed = false;
        
        if (is_collapsed && !_.contains(PYTUNE.Preferences.collapsed_folders, folder_title)) {
            PYTUNE.Preferences.collapsed_folders.push(folder_title);
            changed = true;
        } else if (!is_collapsed && _.contains(PYTUNE.Preferences.collapsed_folders, folder_title)) {
            PYTUNE.Preferences.collapsed_folders = _.without(folders, folder_title);
            changed = true;
        }
        
        if (changed) {
            this.make_request('/profile/set_collapsed_folders', {
                'collapsed_folders': $.toJSON(PYTUNE.Preferences.collapsed_folders)
            }, callback, null);
        }
    },
    ...
    save_exception_change_feed_link: function(feed_id, feed_link, callback, error_callback) {
        ...
        if (PYTUNE.Globals.is_authenticated) {
            this.make_request('/rss_feeds/exception_change_feed_link', {
                ...
                // PYTUNE.log(['save_exception_change_feed_link pre_callback', feed_id, feed_link, data]);
    ...
    save_exception_change_feed_address: function(feed_id, feed_address, callback, error_callback) {
        ...
        if (PYTUNE.Globals.is_authenticated) {
            this.make_request('/rss_feeds/exception_change_feed_address', {
                ...
                // PYTUNE.log(['save_exception_change_feed_address pre_callback', feed_id, feed_address, data]);
                if (data.code < 0 || data.status_code != 200) {
                    return callback(data);
                }
                self.post_refresh_feeds(data, callback);
    ...
    save_pytuneblog_settings: function(data, callback) {
        this.make_request('/social/save_pytuneblog_settings/', data, _.bind(function(response) {
            this.user_profile.set(response.user_profile);
            callback(response);
        }, this));
    },
    ...
    load_public_story_comments: function(story_id, feed_id, callback) {
        this.make_request('/social/public_comments', {
            ...
            var comments = new PYTUNE.Collections.Comments(data.comments);
            callback(comments);
