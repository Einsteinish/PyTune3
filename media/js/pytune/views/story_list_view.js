PYTUNE.Views.StoryListView = Backbone.View.extend({
    ...
    initialize: function() {
        ...
        PYTUNE.reader.$s.$feed_scroll.scroll(this.scroll);
        this.reset_flags();
    },
    ...
    render: function() {
        // console.log(["Rendering story list", PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout')]);
        if (!_.contains(['split', 'full'], PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout'))) return;
        
        var collection = this.collection;
        var stories = _.compact(this.collection.map(function(story) {
            // if (story.story_view) return story;
            var view = new PYTUNE.Views.StoryDetailView({
                model: story,
    ...
    show_correct_explainer: function() {
        if (PYTUNE.assets.preference('feed_view_single_story') && 
            PYTUNE.assets.stories.visible().length) {
            this.show_explainer_single_story_mode();
        } else if (!PYTUNE.assets.stories.visible().length) {
    ...
    show_explainer_no_stories: function() {
        ...
        var counts = PYTUNE.reader.get_unread_count();
        var unread_view_score = PYTUNE.reader.get_unread_view_score();
    ...
    scroll_to_selected_story: function(story, options) {
        options = options || {};
        if (!story) story = PYTUNE.reader.active_story;
        ...
        if (!PYTUNE.assets.preference('animations')) options.immediate = true;
        if (options.scroll_to_comments) {
            $story = $('.NB-feed-story-comments', $story);
        }
        
        if (options.only_if_hidden && PYTUNE.reader.$s.$feed_scroll.isScrollVisible($story, true)) {
            return;
        }
        
        clearTimeout(PYTUNE.reader.locks.scrolling);
        PYTUNE.reader.flags.scrolling_by_selecting_story_title = true;
        var scroll_to = options.scroll_to_top ? 0 : $story;
        PYTUNE.reader.$s.$feed_scroll.stop().scrollTo(scroll_to, { 
            ...
            queue: false, 
            onAfter: function() {
                PYTUNE.reader.locks.scrolling = setTimeout(function() {
                    PYTUNE.reader.flags.scrolling_by_selecting_story_title = false;
                }, 100);
            }
        });
    },
    ...
    show_only_selected_story: function(model) {
        if (!model) model = PYTUNE.reader.active_story;
    ...
    show_no_more_stories: function() {
        ...
        if (!PYTUNE.assets.stories.visible().length) {
            this.show_explainer_no_stories();
            // return;
        }
        
        var pane_height = PYTUNE.reader.$s.$story_pane.height();
        var indicator_position = PYTUNE.assets.preference('lock_mouse_indicator');
        var endbar_height = 20;
        if (indicator_position && 
            _.contains(['full', 'split'], PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout'))) {
            var last_visible_story = _.last(PYTUNE.assets.stories.visible());
        ...
        if (PYTUNE.assets.preference('feed_view_single_story')) {
            var last_story = PYTUNE.assets.stories.last();
    ...
    show_correct_feed_in_feed_title_floater: function(story, hide) {
        var $story, $header;
        var $feed_floater = PYTUNE.reader.$s.$feed_floater;
        story = story || PYTUNE.reader.active_story;
        ...
        if (!hide && story && story.get('story_feed_id') &&
            ...
            this.feed_title_floater = new PYTUNE.Views.StoryDetailView({
                feed_floater: true,
                model: story, 
                el: $new_header
            });
            ...
            this.cache.feed_title_floater_feed_id = story.get('story_feed_id');
            var feed = PYTUNE.assets.get_feed(story.get('story_feed_id'));
    ...
    show_stories_preference_in_feed_view: function(is_creating) {
        if (PYTUNE.reader.active_story && 
            PYTUNE.assets.preference('feed_view_single_story')) {
            this.$el.removeClass('NB-feed-view-feed').addClass('NB-feed-view-story');
            PYTUNE.reader.$s.$feed_stories.scrollTop('0px');
            this.flags['feed_view_positions_calculated'] = false;
        } else {
            this.$el.removeClass('NB-feed-view-story').addClass('NB-feed-view-feed');
            PYTUNE.reader.show_story_titles_above_intelligence_level({'animate': false});
        }
        this.cache.story_pane_position = PYTUNE.reader.$s.$story_pane.offset().top;;
    },
    ...
    show_loading: function(options) {
        ...
        var $feed_scroll = PYTUNE.reader.$s.$feed_scroll;
        this.$('.NB-end-line').remove();
    ...
    end_loading: function() {
        var $endbar = PYTUNE.reader.$s.$feed_scroll.find('.NB-end-line');
        $endbar.remove();
    ...
    append_river_premium_only_notification: function() {
		...
		if (_.contains(['full'], PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout'))) {
    ...
    is_feed_loaded_for_location_fetch: function() {
        var images_begun = PYTUNE.assets.stories.any(function(s) { 
            return !_.isUndefined(s.get('images_loaded')); 
        });
        if (images_begun) {
            var images_loaded = PYTUNE.assets.stories.all(function(s) { 
    ...
    prefetch_story_locations_in_feed_view: function() {
        var self = this;
        var stories = PYTUNE.assets.stories;
        if (!_.contains(['split', 'full'], PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout'))) return;
        if (PYTUNE.assets.preference('feed_view_single_story')) return;
        
        // PYTUNE.log(['Prefetching Feed', this.flags['feed_view_positions_calculated'],  this.is_feed_loaded_for_location_fetch()]);

        if (!PYTUNE.assets.stories.size()) return;
        ...
        if (!this.flags['feed_view_positions_calculated']) {
            ...
            PYTUNE.assets.stories.any(_.bind(function(story) {
                if (!story.story_view) return;
        ...
        if (this.is_feed_loaded_for_location_fetch()) {
            ...
            // PYTUNE.log(['Still loading feed view...', this.cache.feed_view_story_positions_keys.length]);
        }
    },
    ...
    fetch_story_locations_in_feed_view: function(options) {
        ...
        PYTUNE.assets.stories.each(_.bind(function(story) {
            this.determine_feed_view_story_position(story);
        }, this));

        this.flags['feed_view_positions_calculated'] = true;
        // PYTUNE.log(['Feed view entirely loaded', PYTUNE.assets.stories.length + " stories", this.counts['positions_timer']/1000 + " sec delay"]);
    ...
    determine_feed_view_story_position: function(story) {
        ...
        if (story && $story.is(':visible')) {
            ...
            // PYTUNE.log(['Positioning story', position, story.get('story_title')]);
        }
    },
    ...
    check_feed_view_scrolled_to_bottom: function() {
        ...
        var last_story = _.last(PYTUNE.assets.stories.visible());
        if (!last_story || last_story.get('selected')) {
            PYTUNE.reader.load_page_of_feed_stories();
            return;
        }
        if (PYTUNE.assets.preference('feed_view_single_story')) return;
        
        var $last_story = last_story.story_view.$el;
        var container_offset = PYTUNE.reader.$s.$feed_scroll.position().top;
        var full_height = ($last_story.length && $last_story.offset().top) + $last_story.height() - container_offset;
        var visible_height = PYTUNE.reader.$s.$feed_scroll.height() * 2;
        var scroll_y = PYTUNE.reader.$s.$feed_scroll.scrollTop();
        
        // Fudge factor is simply because it looks better at 64 pixels off.
        // PYTUNE.log(['check_feed_view_scrolled_to_bottom', full_height, container_offset, visible_height, scroll_y, PYTUNE.reader.flags['opening_feed']]);
    ...
    check_feed_view_scrolling_from_top: function(scroll_top) {
        var cursor_position = PYTUNE.reader.cache.mouse_position_y + scroll_top;
    ...
    reset_story_positions: function(models) {
        ...
        models.each(_.bind(function(story) {
            ...
            if (!image_count) {
                // PYTUNE.log(["No images", story.get('story_title')]);
                ...
                (function(story, image_count) {
                    story.story_view.$('.NB-feed-story-content img').load(function() {
                        // PYTUNE.log(['Loaded image', story.get('story_title'), image_count]);
                        if (image_count <= 1) {
                            story.set('images_loaded', true);
                        } else {
                            image_count--;
    ...
    handle_mousemove_feed_view: function(e) {
        ...
        if (PYTUNE.assets.preference('feed_view_single_story')) {
            return PYTUNE.reader.hide_mouse_indicator();
        } else {
            PYTUNE.reader.show_mouse_indicator();
        }
        
        if (parseInt(PYTUNE.assets.preference('lock_mouse_indicator'), 10)) {
            return;
        }
        
        // console.log(["mousemove", e.pageY, this.cache.story_pane_position, PYTUNE.reader.cache.mouse_position_y]);
        PYTUNE.reader.cache.mouse_position_y = e.pageY - this.cache.story_pane_position;
        PYTUNE.reader.$s.$mouse_indicator.css('top', PYTUNE.reader.cache.mouse_position_y - 8);
        
        if (this.flags['mousemove_timeout'] ||
            PYTUNE.reader.flags['scrolling_by_selecting_story_title']) {
            return;
        }
        
        var from_top = PYTUNE.reader.cache.mouse_position_y + PYTUNE.reader.$s.$feed_scroll.scrollTop();
        var offset = this.cache.offset || 0;
        if (PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout') == 'full' && !this.cache.offset) {
    ...
    scroll: function(elem, e) {
        var self = this;
        var story_view = PYTUNE.reader.story_view;
        var offset = this.cache.offset || 0;
        if (PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout') == 'full' && !this.cache.offset) {
        ...
        if ((story_view == 'feed' ||
             (story_view == 'page' && PYTUNE.reader.flags['page_view_showing_feed_view'])) &&
            !PYTUNE.reader.flags['scrolling_by_selecting_story_title'] &&
            !PYTUNE.assets.preference('feed_view_single_story')) {
            var scroll_top = PYTUNE.reader.$s.$feed_scroll.scrollTop();
            var from_top = PYTUNE.reader.cache.mouse_position_y + scroll_top;
            var position = from_top - offset;
            var positions = this.cache.feed_view_story_positions_keys;
            var closest = $.closest(position, positions);
            var story = this.cache.feed_view_story_positions[positions[closest]];
            ...
            if (!story) return;
            // PYTUNE.log(["Scroll Feed", PYTUNE.reader.cache.mouse_position_y, PYTUNE.reader.$s.$feed_scroll.scrollTop(), from_top, offset, position, closest, story.get('story_title')]);
            if (!story.get('selected')) {
                story.set('selected', true, {selected_by_scrolling: true, mouse: true, immediate: true});
            }

            this.check_feed_view_scrolled_to_bottom();
            if (scroll_top < 10 || PYTUNE.assets.preference('mark_read_on_scroll_titles')) {
                this.check_feed_view_scrolling_from_top(scroll_top);
            }
        }
        
        if ((PYTUNE.reader.flags['river_view'] || PYTUNE.reader.flags['social_view']) &&
            !PYTUNE.assets.preference('feed_view_single_story')) {
            var story;
            if (PYTUNE.reader.flags['scrolling_by_selecting_story_title']) {
                story = this.active_story;
            } else {
                var from_top = Math.max(1, PYTUNE.reader.$s.$feed_scroll.scrollTop());
                var positions = this.cache.feed_view_story_positions_keys;
