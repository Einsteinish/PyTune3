PYTUNE.Views.StoryCommentsView = Backbone.View.extend({
    ...
    render_teaser: function() {
        ...
        _.each(this.model.get('shared_by_friends'), function(user_id) { 
            ...
            var profile_thumb = PYTUNE.Views.ProfileThumb.create(user_id);
    ...
    render_comments_friends: function() {
        ...
        this.model.friend_comments.each(_.bind(function(comment) {
            var $comment = new PYTUNE.Views.StoryComment({
                model: comment, 
                story: this.model,
                friend_comment: true
            }).render().el;
    ...
    render_comments_public: function() {
        ...
        if (PYTUNE.assets.preference('hide_public_comments')) {
    ...
    load_public_story_comments: function() {
        var following_user_ids = PYTUNE.assets.user_profile.get('following_user_ids');
        this.$(".NB-story-comments-expand-icon").addClass("NB-loading");
        
        PYTUNE.assets.load_public_story_comments(this.model.id, this.model.get('story_feed_id'), _.bind(function(comments) {
            this.$(".NB-story-comments-expand-icon").addClass("NB-loading");
