PYTUNE.Views.StoryComment = Backbone.View.extend({
    ...
    initialize: function(options) {
        ...
        if (!this.options.on_social_page) {
            this.user = PYTUNE.assets.user_profiles.find(this.model.get('user_id'));
    ...
    render: function() {
        ...
        var liked = _.contains(this.model.get('liking_users'), PYTUNE.Globals.user_id);
        ...
        var $comment = $.make('div', { className: (this.options.friend_share ? "NB-story-comment-friend-share" : "") }, [
            ...
            $.make('div', { className: 'NB-story-comment-author-container' }, [
                (this.model.get('source_user_id') && $.make('div', { className: 'NB-story-comment-reshares' }, [
                    PYTUNE.Views.ProfileThumb.create(this.model.get('source_user_id')).render().el
                ])),
                ...
                $.make('div', { className: 'NB-story-comment-reply-button' }, [
                    $.make('div', { className: 'NB-story-comment-reply-button-wrapper' }, [
                        (this.user.get('protected') && $.make('img', { src: PYTUNE.Globals.MEDIA_URL + 'img/icons/circular/g_icn_lock.png' })),
                        'reply'
                    ])
                ]),
                (this.model.get('user_id') == PYTUNE.Globals.user_id && $.make('div', { className: 'NB-story-comment-edit-button NB-story-comment-share-edit-button' }, [
                ...
                (!has_likes && this.model.get('user_id') != PYTUNE.Globals.user_id && $.make('div', { className: 'NB-story-comment-likes NB-left' }, [
    ...
    make_story_share_comment_replies: function() {
        ...
        var user_id = PYTUNE.Globals.user_id;
        var $replies = this.model.replies.map(_.bind(function(reply) {
            if (!PYTUNE.assets.get_user(reply.get('user_id'))) return;
            return new PYTUNE.Views.StoryCommentReply({model: reply, comment: this}).render().el;
        }, this));
    ...
    render_liking_users: function() {
        ...
        _.each(this.model.get('liking_users'), function(user_id) { 
            if (!PYTUNE.assets.get_user(user_id)) return;
            var $thumb = PYTUNE.Views.ProfileThumb.create(user_id).render().el;
            $users.append($thumb);
        });
    ...
    open_social_profile_modal: function() {
        PYTUNE.reader.open_social_profile_modal(this.model.get("user_id"));
    },
    ...
    open_reply: function(options) {
        options = options || {};
        var current_user = PYTUNE.assets.user_profile;
    ...
    save_social_comment_reply: function() {
        ...
        PYTUNE.assets.save_comment_reply(this.options.story.id, this.options.story.get('story_feed_id'), 
    ...
    delete_social_comment_reply: function() {
        ...
        PYTUNE.assets.delete_comment_reply(this.options.story.id,
    ...
    like_comment: function() {
        ...
        var comment_user_id = this.model.get('user_id');
        var liked = _.contains(liking_user_ids, PYTUNE.Globals.user_id);
        
        if (!liked) {
            this.model.set('liking_users', _.union(liking_user_ids, PYTUNE.Globals.user_id));
            PYTUNE.assets.like_comment(this.options.story.id, 
                                         this.options.story.get('story_feed_id'),
                                         comment_user_id);
        } else {
            this.model.set('liking_users', _.without(liking_user_ids, PYTUNE.Globals.user_id));
            PYTUNE.assets.remove_like_comment(this.options.story.id, 
                                                this.options.story.get('story_feed_id'),
                                                comment_user_id);
        }
    }
