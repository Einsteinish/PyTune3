...
def set_preference(request):
    print 'KHong profile_views set_preference()'
    code = 1
    message = ''
...
def get_preference(request):
    print 'KHong profile_views get_preference()'
    code = 1
...
@csrf_protect
def login(request):
    print 'KHong profile_views login()'
    form = LoginForm()

    if request.method == "POST":
        form = LoginForm(data=request.POST)
        print 'KHong profile_views LoginForm()'
        if form.is_valid():
            login_user(request, form.get_user())
            print 'KHong profile_views login_user()'
...
def set_account_settings(request):
    print 'KHong profile_views signup()'
    code = -1
    message = 'OK'
...
def clear_view_setting(request):
    print 'KH profile_views clear_view_setting()'
    code = 1
...
def get_view_setting(request):
    print 'KH profile_views get_view_setting()'
    code = 1
    feed_id = request.POST['feed_id']
    view_settings = json.decode(request.user.profile.view_settings)
...
def set_collapsed_folders(request):
    print 'KH profile_views get_view_setting()'
    code = 1
    collapsed_folders = request.POST['set_collapsed_folders']
...
def paypal_form(request):
    print 'KH profile_views paypal_form()'
    ...
    paypal_dict = {
        ...
        "item_name": "PyTune Premium Account",
...
def profile_is_premium(request):
    print 'KH profile_views profile_is_premium()'
    # Check tries
    code = 0
...
def stripe_form(request):
    print 'KH profile_views stripe_form()'
    user = request.user
...
def load_activities(request):
    print 'KH profile_views load_activities()'
