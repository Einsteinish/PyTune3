PYTUNE.Models.Story = Backbone.Model.extend({
    ...
    populate_comments: function(story, collection) {
        this.friend_comments = new PYTUNE.Collections.Comments(this.get('friend_comments'));
        this.friend_shares = new PYTUNE.Collections.Comments(this.get('friend_shares'));
        this.public_comments = new PYTUNE.Collections.Comments(this.get('public_comments'));
    },
    ...
    score: function() {
        if (PYTUNE.reader.flags['starred_view']) {
            ...
            return PYTUNE.utils.compute_story_score(this);
        }
    },
    ...
    formatted_short_date: function() {
        ...
        var dateformat = PYTUNE.assets.preference('dateformat');
    ...
    mark_read: function(options) {
        return PYTUNE.assets.stories.mark_read(this, options);
    },
    ...
    open_share_dialog: function(e, view) {
        if (view == 'title') {
            ...
            PYTUNE.reader.show_manage_menu('story', $story_title, {story_id: this.id});
            PYTUNE.reader.show_confirm_story_share_menu_item(this.id);
        } else {
    ...
    change_starred: function() {
        if (this.get('starred')) {
            PYTUNE.assets.mark_story_as_starred(this.id);
        } else {
            PYTUNE.assets.mark_story_as_unstarred(this.id);
        }
    },
    ...
    all_tags: function() {
        ...
        var existing_tags = PYTUNE.assets.starred_feeds.all_tags();
...
PYTUNE.Collections.Stories = Backbone.Collection.extend({
    
    model: PYTUNE.Models.Story,
    ...
    mark_read: function(story, options) {
        options = options || {};
        var delay = PYTUNE.assets.preference('read_story_delay');
        ...
        var _mark_read = _.bind(function() {
            if (!delay || (delay && this.active_story.id == story.id)) {
                var feed = PYTUNE.assets.get_feed(PYTUNE.reader.active_feed);
                if (!feed) {
                    feed = PYTUNE.assets.get_feed(story.get('story_feed_id'));
                }
                PYTUNE.assets.mark_story_hash_as_read(story, _.bind(function(read) {
    ...
    mark_unread: function(story, options) {
        options = options || {};
        PYTUNE.assets.mark_story_as_unread(story.id, story.get('story_feed_id'), _.bind(function(read) {
    ...
    update_read_count: function(story, options) {
        ...
        var story_unread_counter  = PYTUNE.app.story_unread_counter;
        var unread_view           = PYTUNE.reader.get_unread_view_name();
        var active_feed           = PYTUNE.assets.get_feed(PYTUNE.reader.active_feed);
        var story_feed            = PYTUNE.assets.get_feed(story.get('story_feed_id'));
        var friend_feeds          = PYTUNE.assets.get_friend_feeds(story);
    ...
    select_previous_story: function() {
        if (this.previous_stories_stack.length) {
            ...
            if (previous_story.get('selected') ||
                previous_story.score() < PYTUNE.reader.get_unread_view_score()) {
    ...
    visible: function(score) {
        score = _.isUndefined(score) ? PYTUNE.reader.get_unread_view_score() : score;
    ...
    detect_selected_story: function(selected_story, selected) {
        if (selected) {
            ...
            PYTUNE.reader.active_story = selected_story;
