PYTUNE.Views.SocialPageStory = Backbone.View.extend({
    ...
    initialize: function() {
        ...
        this.comments_view = new PYTUNE.Views.SocialPageComments({
        ...
        this.shares_view = new PYTUNE.Views.SocialPageSharesView({
    ...
    mark_story_as_shared: function(options) {
        ...
        var source_user_id = PYTUNE.Globals.pytuneblog_user_id;
        var relative_user_id = PYTUNE.Globals.pytuneblog_user_id;
        var post_to_services = _.compact([
            PYTUNE.assets.preference('post_to_twitter') && 'twitter',
            PYTUNE.assets.preference('post_to_facebook') && 'facebook'
        ]);
        ...
        PYTUNE.assets.mark_story_as_shared(data, _.bind(this.post_share_story, this, true), _.bind(function(data) {
            this.post_share_error(data, true);
        }, this));
        
        if (PYTUNE.reader) {
            PYTUNE.reader.blur_to_page();
        }
    },
    ...
    post_share_error: function(data, shared) {
        ...
        PYTUNE.log(["post_share_error", data, shared, message, $share_button, $unshare_button, $share_button_menu, $error]);
    },
    ...
    mark_story_as_unshared: function(options) {
        ...
        var params = {
            story_id: this.model.id, 
            story_feed_id: this.model.get('story_feed_id'),
            relative_user_id: PYTUNE.Globals.pytuneblog_user_id
        };
        PYTUNE.assets.mark_story_as_unshared(params, _.bind(this.post_share_story, this, false), _.bind(function(data) {
    ...
    check_crosspost_buttons: function() {
        ...
        if (!PYTUNE.user_social_services) return;
        
        if (PYTUNE.user_social_services.twitter &&
            PYTUNE.user_social_services.twitter.twitter_uid) {
            $twitter.removeClass('NB-hidden');
        }
        if (PYTUNE.user_social_services.facebook &&
            PYTUNE.user_social_services.facebook.facebook_uid) {
            $facebook.removeClass('NB-hidden');
        }
        
        $twitter.toggleClass('NB-active', !!PYTUNE.assets.preference('post_to_twitter'));
        $facebook.toggleClass('NB-active', !!PYTUNE.assets.preference('post_to_facebook'));
    },
    ...
    toggle_twitter: function() {
        ...
        if (PYTUNE.assets.preference('post_to_twitter')) {
            PYTUNE.assets.preference('post_to_twitter', false);
        } else {
            PYTUNE.assets.preference('post_to_twitter', true);
        }
        
        $twitter_button.toggleClass('NB-active', PYTUNE.assets.preference('post_to_twitter'));
        this.reset_posting_label();
    },
    ...
    toggle_facebook: function() {
        ...
        if (PYTUNE.assets.preference('post_to_facebook')) {
            PYTUNE.assets.preference('post_to_facebook', false);
        } else {
            PYTUNE.assets.preference('post_to_facebook', true);
        }
        
        $facebook_button.toggleClass('NB-active', PYTUNE.assets.preference('post_to_facebook'));
        this.reset_posting_label();
    },
    ...
    show_posting_label: function(twitter, facebook) {
        var social_services = PYTUNE.user_social_services || {};
        var $text = this.$('.NB-story-comment-crosspost-text');
        twitter = twitter || (social_services.twitter && social_services.twitter.twitter_uid && PYTUNE.assets.preference('post_to_twitter'));
        facebook = facebook || (social_services.facebook && social_services.facebook.facebook_uid && PYTUNE.assets.preference('post_to_facebook'));
