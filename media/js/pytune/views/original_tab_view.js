PYTUNE.Views.OriginalTabView = Backbone.View.extend({
    ...
    find_story_in_feed_iframe: function(story) {
        ...
        if (!$stories.length) {
            ...
            this.cache.iframe['headers'].each(function() {
                var pos = $(this).text().replace(/&nbsp;|[^a-z0-9-,]/gi, '')
                             .indexOf(title);
                // PYTUNE.log(['Search headers', title, pos, $(this), $(this).text()]);
                if (pos >= 0) {
                    $stories.push($(this));
                    return false;
                }
        ...
        if (!$stories.length) {
            ...
            this.cache.iframe['posts'].each(function() {
                pos = $(this).text().replace(/&nbsp;|[^a-z0-9-,]/gi, '')
                             .indexOf(title);
                // PYTUNE.log(['Search .post', title, pos, $(this), $(this).text().replace(/&nbsp;|[^a-z0-9-,]/gi, '')]);
                if (pos >= 0) {
                    $stories.push($(this));
                    return false;
                }
        ...
        // PYTUNE.log(['Found stories', $stories.length, $same_size_stories.length, $same_size_stories, story.get('story_title')]);
        ...
        if ($story && $story.length) {
            ...
            if (!this.flags['iframe_view_not_busting']) {
                var feed_id = PYTUNE.reader.active_feed;
                _.delay(_.bind(function() {
                    if (feed_id == PYTUNE.reader.active_feed) {
    ...
    scroll_to_selected_story: function(story, options) {
        ...
        if (!PYTUNE.assets.preference('animations') ||
            PYTUNE.reader.story_view == 'feed' ||
            PYTUNE.reader.story_view == 'story' ||
            PYTUNE.reader.flags['page_view_showing_feed_view']) options.immediate = true;

        // PYTUNE.log(["Scroll in Original", story.get('story_title'), options]);
    ...
    prefetch_story_locations_in_story_frame: function() {
        ...
        PYTUNE.log(['Prefetching Original', !this.flags['iframe_fetching_story_locations'], !this.flags['iframe_story_locations_fetched']]);
        if (!this.flags['iframe_fetching_story_locations'] 
            ...
            // PYTUNE.log(['last_story', last_story_index, last_story_position, last_story, $last_story]);
            ...
            PYTUNE.assets.stories.any(_.bind(function(story, i) {
                if (last_story_same_position && i < last_story_index) return true; 
                
                var $story = this.find_story_in_feed_iframe(story, $iframe);
                // PYTUNE.log(['Pre-fetching', i, last_story_index, last_story_same_position, $story, story.get('story_title')]);
                if (!$story || 
                    !$story.length || 
                    ...
                    if ($story && $story.length) {
                        PYTUNE.log(['Prefetch break on position too far', parseInt($story.offset().top, 10), this.cache['prefetch_iteration']*4000]);
                        return true;
                    }
    ...
    fetch_story_locations_in_story_frame: function($iframe, options) {
        ...
        PYTUNE.assets.stories.any(_.bind(function(story, i) {
            if (story.get('story_feed_id') == PYTUNE.reader.active_feed || 
                "social:" + story.get('social_user_id') == PYTUNE.reader.active_feed) {
                var $story = this.find_story_in_feed_iframe(story, $iframe);
                // PYTUNE.log(['Fetching story', i, story.get('story_title'), $story]);
            
                if (self.cache['story_misses'] > 5) {
                    // PYTUNE.log(['iFrame view entirely loaded', self.cache['story_misses'], self.cache.iframe_stories]);
            ...
            } else if (story && story.get('story_feed_id') != PYTUNE.reader.active_feed &&
                       "social:" + story.get('social_user_id') != PYTUNE.reader.active_feed) {
                PYTUNE.log(['Switched off iframe early', PYTUNE.reader.active_feed, story.get('story_feed_id'), story.get('social_user_id')]);
                return true;
            }
        }, this));
        
        PYTUNE.log(['Original view entirely loaded', _.keys(self.cache.iframe_stories).length + " stories", this.counts['positions_timer']/1000 + " sec delay"]);
    ...
    reset_story_positions: function(models) {
        ...
        if (PYTUNE.reader.story_view != 'page') return;
        
        this.flags['iframe_fetching_story_locations'] = false;
        this.flags['iframe_story_locations_fetched'] = false;
        
        if (PYTUNE.reader.flags['story_titles_loaded']) {
    ...
    unload_feed_iframe: function() {
        ...
        this.flags['iframe_story_locations_fetched'] = false;
        PYTUNE.reader.flags['iframe_prevented_from_loading'] = false;
    ...
    load_feed_iframe: function(feed_id) {
        feed_id = feed_id || PYTUNE.reader.active_feed;
        var self = this;

        this.flags['iframe_loaded'] = true;
        
        var page_url = '/reader/page/'+feed_id;
        if (PYTUNE.reader.flags['social_view']) {
            var feed = PYTUNE.assets.get_feed(feed_id);
            page_url = feed.get('page_url');
        }
        
        PYTUNE.reader.flags.iframe_scroll_snap_back_prepared = true;
        ...
        this.$el.ready(function() {
            ...
            if (feed_id != PYTUNE.reader.active_feed) {
                PYTUNE.log(["Switched feed, unloading iframe"]);
            ...
            setTimeout(function() {
                self.$el.load(function() {
                    ...
                    self.cache.iframe_feed_id = PYTUNE.reader.active_feed;
                });
            }, 50);
            self.flags['iframe_scroll_snapback_check'] = setInterval(function() {
                // PYTUNE.log(['Checking scroll', self.cache.iframe_scroll, self.flags.iframe_scroll_snap_back_prepared, self.flags['iframe_scroll_snapback_check']]);
            ...
            if (feed_iframe_src && feed_iframe_src.indexOf('/reader/page/'+feed_id) != -1) {
                var iframe_link_attacher = function() {
                    ...
                    // PYTUNE.log(['Finding links', self.iframe_link_attacher_num_links, num_links]);
                    if (self.iframe_link_attacher_num_links != num_links) {
                        // PYTUNE.log(['Found new links', num_links, self.iframe_link_attacher_num_links]);
    ...
    return_to_snapback_position: function(iframe_loaded) {
        ...
        if (this.cache.iframe_scroll
            ...
            PYTUNE.log(['Snap back, loaded, scroll', this.cache.iframe_scroll]);
    ...
    setup_feed_page_iframe_load: function() {
        this.$el.load(_.bind(function() {
            ...
            this.setup_events();
            if (PYTUNE.reader.flags['story_titles_loaded']) {
                PYTUNE.log(['iframe loaded, titles loaded']);
                this.fetch_story_locations_in_story_frame();
            }
            // try {
                var $iframe_contents = this.$el.contents();
                $iframe_contents.find('a')
                    ...
                    if (href && href.indexOf('#') == 0) {
                        ...
                        var $footnote = $('a[name='+href.substr(1)+'], [id='+href.substr(1)+']',
                                          $iframe_contents);
                        PYTUNE.log(['Footnote', $footnote, href, href.substr(1)]);
        ...
        if (PYTUNE.reader.flags['story_titles_loaded']) {
            PYTUNE.log(['iframe loaded, titles loaded (early)']);
            this.fetch_story_locations_in_story_frame();
        }
    ...
    taskbar_show_return_to_page: function() {
        _.delay(_.bind(function() {
            ...
            try {
                PYTUNE.log(['return', this.$el.contents().find('body')]);
        ...
        this.locks.iframe_buster_buster = setInterval(function() {
            if (prevent_bust > 0) {
                ...
                if (self.flags['iframe_story_locations_fetched'] && 
                    ...
                    PYTUNE.reader.active_feed) {
                  $('.NB-feed-frame').attr('src', '');
                  window.top.location = '/reader/buster';
                  PYTUNE.reader.switch_taskbar_view('feed');
                }
            }
        }, 1);
    },
    ...
    handle_scroll_feed_iframe: function(e) {
        if (PYTUNE.reader.story_view == 'page'
            && !PYTUNE.reader.flags['page_view_showing_feed_view']
            && !PYTUNE.reader.flags['scrolling_by_selecting_story_title']) {
            var from_top = PYTUNE.reader.cache.mouse_position_y + this.$el.contents().scrollTop();
            ...
            if (!story) return;
            if (story.score() < PYTUNE.reader.get_unread_view_score()) return;
            // PYTUNE.log(['Scroll iframe', from_top, closest, positions[closest], this.cache.iframe_story_positions[positions[closest]]]);
            ...
            if (!this.flags.iframe_scroll_snap_back_prepared) {
                this.cache.iframe_scroll = from_top - PYTUNE.reader.cache.mouse_position_y;
            }
            this.flags.iframe_scroll_snap_back_prepared = false;
        }
    },
    ...
    handle_mousemove_iframe_view: function(e) {
        ...
        // PYTUNE.log(["mousemove", e, scroll_top, e.pageY]);
        PYTUNE.reader.cache.mouse_position_y = e.pageY - scroll_top;
        ...
        var closest = $.closest(from_top, positions);
        var story = this.cache.iframe_story_positions[positions[closest]];
        // PYTUNE.log(["mousemove", story, from_top, positions[closest], this.cache.iframe_story_positions]);
        // this.flags['mousemove_timeout'] = true;
        if (!story) return;
        if (story.score() < PYTUNE.reader.get_unread_view_score()) {
