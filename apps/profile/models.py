...
class Profile(models.Model):
    ...
    def send_new_user_email(self):
        ...
        subject = "Welcome to PyTune, %s" % (self.user.username)
        msg     = EmailMultiAlternatives(subject, text, 
                                         from_email='PyTune <%s>' % settings.HELLO_EMAIL,
    ...
    def send_opml_export_email(self, reason=None, force=False):
        ...
        subject = "Backup OPML file of your PyTune sites"
        filename= 'PyTune Subscriptions - %s.xml' % datetime.datetime.now().strftime('%Y-%m-%d')
        msg     = EmailMultiAlternatives(subject, text, 
                                         from_email='PyTune <%s>' % settings.HELLO_EMAIL,
                                         to=['%s <%s>' % (user, user.email)])
        msg.attach_alternative(html, "text/html")
    ...
    def send_first_share_to_pytuneblog_email(self, force=False):
        ...
        params = {
            ...
            'pytuneblog_url': social_profile.pytuneblog_url,
            'pytuneblog_rss': social_profile.pytuneblog_rss
        }
        user    = self.user
        text    = render_to_string('mail/email_first_share_to_pytuneblog.txt', params)
        html    = render_to_string('mail/email_first_share_to_pytuneblog.xhtml', params)
        subject = "Your shared stories on PyTune are available on your Pytuneblog"
        msg     = EmailMultiAlternatives(subject, text, 
                                         from_email='PyTune <%s>' % settings.HELLO_EMAIL,
                                         to=['%s <%s>' % (user, user.email)])
        msg.attach_alternative(html, "text/html")
        ...
        logging.user(self.user, "~BB~FM~SBSending first share to pytuneblog email to: %s" % self.user.email)
...
Sincerely,
PyTune""" % {'user': self.user.username, 'feeds': subs.count()}
        ...
        subject = "Thanks for going premium on PyTune!"
        msg     = EmailMultiAlternatives(subject, text, 
                                         from_email='PyTune <%s>' % settings.HELLO_EMAIL,
                                         to=['%s <%s>' % (user, user.email)])
        msg.attach_alternative(html, "text/html")
    ...
    def send_forgot_password_email(self, email=None):
        ...
        subject = "Forgot your password on PyTune?"
        msg     = EmailMultiAlternatives(subject, text, 
                                         from_email='PyTune <%s>' % settings.HELLO_EMAIL,
                                         to=['%s <%s>' % (user, user.email)])
        msg.attach_alternative(html, "text/html")
    ...
    def send_new_user_queue_email(self, force=False):
        ...
        subject = "Your free account is now ready to go on PyTune"
        msg     = EmailMultiAlternatives(subject, text, 
                                         from_email='PyTune <%s>' % settings.HELLO_EMAIL,
                                         to=['%s <%s>' % (user, user.email)])
        msg.attach_alternative(html, "text/html")
    ...
    def send_upload_opml_finished_email(self, feed_count):
        ...
        subject = "Your OPML upload is complete. Get going with PyTune!"
        msg     = EmailMultiAlternatives(subject, text, 
                                         from_email='PyTune <%s>' % settings.HELLO_EMAIL,
                                         to=['%s <%s>' % (user, user.email)])
        msg.attach_alternative(html, "text/html")
    ...
    def send_import_reader_finished_email(self, feed_count):
        ...
        subject = "Your Google Reader import is complete. Get going with PyTune!"
        msg     = EmailMultiAlternatives(subject, text, 
                                         from_email='PyTune <%s>' % settings.HELLO_EMAIL,
                                         to=['%s <%s>' % (user, user.email)])
        msg.attach_alternative(html, "text/html")
    ...
    def send_import_reader_starred_finished_email(self, feed_count, starred_count):
        ...
        subject = "Your Google Reader starred stories import is complete. Get going with PyTune!"
        msg     = EmailMultiAlternatives(subject, text, 
                                         from_email='PyTune <%s>' % settings.HELLO_EMAIL,
                                         to=['%s <%s>' % (user, user.email)])
        msg.attach_alternative(html, "text/html")
    ...
    def send_launch_social_email(self, force=False):
        ...
        subject = "PyTune is now a social news reader"
        msg     = EmailMultiAlternatives(subject, text, 
                                         from_email='PyTune <%s>' % settings.HELLO_EMAIL,
                                         to=['%s <%s>' % (user, user.email)])
        msg.attach_alternative(html, "text/html")
    ...
    def send_premium_expire_grace_period_email(self, force=False):
        ...
        subject = "Your premium account on PyTune has one more month!"
        msg     = EmailMultiAlternatives(subject, text, 
                                         from_email='PyTune <%s>' % settings.HELLO_EMAIL,
                                         to=['%s <%s>' % (user, user.email)])
        msg.attach_alternative(html, "text/html")
    ...
    def send_premium_expire_email(self, force=False):
        ...
        subject = "Your premium account on PyTune has expired"
        msg     = EmailMultiAlternatives(subject, text, 
                                         from_email='PyTune <%s>' % settings.HELLO_EMAIL,
                                         to=['%s <%s>' % (user, user.email)])
        msg.attach_alternative(html, "text/html")
...
class MRedeemedCode(mongo.Document):
    ...
    def redeem(cls, user, gift_code):
        pytune_gift_code = MGiftCode.objects.filter(gift_code__iexact=gift_code)
        if pytune_gift_code:
            pytune_gift_code = pytune_gift_code[0]
            PaymentHistory.objects.create(user=user,
                                          payment_date=datetime.datetime.now(),
                                          payment_amount=pytune_gift_code.payment_amount,
                                          payment_provider='pytune-gift')
