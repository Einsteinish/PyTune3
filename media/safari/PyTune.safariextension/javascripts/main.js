 ...
 *
 * Based on: PyTune Notifier by Espen Helgedagsrud (kleevah@zawiarr.com)
 *   (https://addons.opera.com/en/extensions/details/pytune-notifier/)
 *   Based on Google Reader Notifier by al007 
 *    (http://addons.opera.com/extensions/details/google-reader-notifier/)
 */

var PYTUNE_DOMAIN   = "pytune.com";
var PYTUNE_PATH     = "/folder/everything";
var GET_UNREAD_UPDATE = "/reader/refresh_feeds";
var API_LOGIN_PATH = "/api/login";

var TITLE_REGEX_SINGLE = /\((\d+)\)\sPyTune/g;
var TITLE_REGEX_DOUBLE = /\((\d+)\/(\d+)\)\sPyTune/g;
...
// Fetch the unread count from the PyTune API using JSON.
...
// Returns the URL for PyTune with proper prefix.
function getURL(){
  var dprefix;
  dprefix  = (useSSL) ? "https://" : "http://";
  
  return dprefix + PYTUNE_DOMAIN;
}
...
function openPyTune(event) {
  if (event.target.identifier !== "menuOpenPyTune") return;
  if (event.command !== 'openPyTune') return;
  if (isTabFocused()) return;
  if (isTabOpen()) return;

	var tab = safari.application.activeBrowserWindow.openTab();
	tab.url = getURL() + PYTUNE_PATH;
}


var settingsListener = safari.extension.settings.addEventListener("change", settingChanged, false);
var openListener = safari.application.addEventListener("command", openPyTune, false);
