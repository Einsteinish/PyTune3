PYTUNE.ReaderFriends = function(options) {
    var defaults = {
        width: 800
    };
...
PYTUNE.ReaderFriends.prototype = new PYTUNE.Modal;

_.extend(PYTUNE.ReaderFriends.prototype, {
    ...
    fetch_friends: function(callback) {
        ...
        PYTUNE.assets.fetch_friends(_.bind(function(data) {
            this.profile = PYTUNE.assets.user_profile.clone();
    ...
    make_find_friends_and_services: function() {
        ...
        _.each(['twitter', 'facebook', 'appdotnet'], _.bind(function(service) {
            ...
            if (this.services && this.services[service][service+'_uid']) {
                ...
                $service = $.make('div', { className: 'NB-friends-service NB-connected NB-friends-service-'+service + (this.services[service].syncing ? ' NB-friends-service-syncing' : '') }, [
                    $.make('div', { className: 'NB-friends-service-title' }, PYTUNE.utils.service_name(service)),
                    $.make('div', { className: 'NB-friends-service-connect NB-modal-submit-button NB-modal-submit-grey' }, [
                        $.make('img', { src: PYTUNE.Globals.MEDIA_URL + '/img/reader/' + service + '_service.png' }),
                ...
                $service = $.make('div', { className: 'NB-friends-service NB-friends-service-'+service }, [
                    ...
                    $.make('div', { className: 'NB-friends-service-connect NB-modal-submit-button NB-modal-submit-green' }, [
                        $.make('img', { src: PYTUNE.Globals.MEDIA_URL + '/img/reader/' + service + '_service_off.png' }),
                        'Find ' + PYTUNE.utils.service_name(service) + ' Friends'
                    ])
                ]);
            }
        ...
        $('.NB-friends-search').html($.make('div', [
            $.make('div', { className: "NB-module-search-input NB-module-search-people" }, [
                ...
                $.make('label', { 'for': "NB-friends-search-input" }, [
                    $.make('img', { src: PYTUNE.Globals.MEDIA_URL + "img/reader/search_icon2.png" })
                ]),
        ...
        if (this.recommended_users.length) {
            _.each(this.recommended_users, function(profile) {
                var profile_model = new PYTUNE.Models.User(profile);
                $profile_badge = new PYTUNE.Views.SocialProfileBadge({
                    model: profile_model
                });
    ...
    make_profile_section: function() {
        ...
        //         $.make('img', { src: PYTUNE.Globals['MEDIA_URL']+'img/icons/silk/eye.png', style: 'padding-left: 10px' }),
        //         $.make('img', { src: PYTUNE.Globals['MEDIA_URL']+'img/icons/silk/eye.png' })
        //     ]);
        // } else {
            $profile_badge = new PYTUNE.Views.SocialProfileBadge({
                model: profile,
    ...
    make_followers_tab: function() {
        ...
        if (this.profile.get('follower_count') <= 0) {
            ...
            $tab.append($heading);
            PYTUNE.assets.follower_profiles.each(_.bind(function(profile) {
                $tab.append(new PYTUNE.Views.SocialProfileBadge({model: profile}));
            }, this));
        }
    },
    ...
    make_following_tab: function() {
        ...
        if (this.profile.get('following_count') <= 0) {
            ...
            $tab.append($heading);
            PYTUNE.assets.following_profiles.each(_.bind(function(profile) {
                $tab.append(new PYTUNE.Views.SocialProfileBadge({model: profile}));
            }, this));
        }
    },
    ...
    search_for_friends: function(query) {
        ...
        PYTUNE.assets.search_for_friends(query, _.bind(function(data) {
            ...
            $badges.html($.make('div', _.map(data.profiles, function(profile) {
                var user = new PYTUNE.Models.User(profile);
                return new PYTUNE.Views.SocialProfileBadge({model: user});
            })));
    ...
    handle_click: function(elem, e) {
        ...
        $.targetIs(e, { tagSelector: '.NB-friends-profile-link' }, function($t, $p) {
            ...
            self.close(function() {
                PYTUNE.reader.open_profile_editor_modal();
            });
        });
    },
