...
class MSocialProfile(mongo.Document):
    ...
    pytuneblog_title       = mongo.StringField(max_length=256)
    ...
    @property
    def pytuneblog_url(self):
        return "http://%s.%s/" % (
            self.username_slug,
            Site.objects.get_current().domain.replace('www.', ''))
    
    @property
    def pytuneblog_rss(self):
        return "%s%s" % (self.pytuneblog_url, reverse('shared-stories-rss-feed', 
    ...
    @property
    def title(self):
        return self.pytuneblog_title if self.pytuneblog_title else self.username + "'s pytuneblog"
    ...
    def send_email_for_new_follower(self, follower_user_id):
        ...
        subject = "%s is now following your Pytuneblog on PyTune!" % follower_profile.username
        msg     = EmailMultiAlternatives(subject, text, 
                                         from_email='PyTune <%s>' % settings.HELLO_EMAIL,
    ...
    def send_email_for_follow_request(self, follower_user_id):
        ...
        subject = "%s has requested to follow your Pytuneblog on PyTune" % follower_profile.username
        msg     = EmailMultiAlternatives(subject, text, 
                                         from_email='PyTune <%s>' % settings.HELLO_EMAIL,
                                         to=['%s <%s>' % (user.username, user.email)])
        msg.attach_alternative(html, "text/html")
...
class MSharedStory(mongo.Document):
    ...
    def trim_old_stories(cls, stories=10, days=90, dryrun=False):
        ...
        stats = settings.MONGODB.pytune.shared_stories.aggregate([{
            "$group": {
                "_id":      "$user_id",
                "stories":  {"$sum": 1},
            },
    ...
    def get_shared_stories_from_site(cls, feed_id, user_id, story_url, limit=3):
        ...
        same_stories = [{
            ...
            "pytuneblog_permalink": story.pytuneblog_permalink(),
    ...
    def pytuneblog_permalink(self):
        profile = MSocialProfile.get_user(self.user_id)
        return "%sstory/%s/%s" % (
            profile.pytuneblog_url,
    ...
    def generate_post_to_service_message(self, truncate=None, include_url=True):
        ...
        if not message or len(message) < 1:
            ...
            if include_url:
                message += " " + self.pytuneblog_permalink()
        elif include_url:
            if truncate:
    ...
    def send_emails_for_new_reply(self, reply_id):
        ...
        for user_id in notify_user_ids:
            ...
            subject = "%s replied to you on \"%s\" on PyTune" % (reply_user.username, story_title)
            msg     = EmailMultiAlternatives(subject, text, 
                                             from_email='PyTune <%s>' % settings.HELLO_EMAIL,
                                             to=['%s <%s>' % (user.username, user.email)])
            msg.attach_alternative(html, "text/html")
    ...
    def send_email_for_reshare(self):
        ...
        subject = "%s re-shared \"%s\" from you on PyTune" % (reshare_user.username, story_title)
        msg     = EmailMultiAlternatives(subject, text, 
                                         from_email='PyTune <%s>' % settings.HELLO_EMAIL,
    ...
    def calculate_image_sizes(self, force=False):
        ...
        headers = {
            'User-Agent': 'PyTune Image Fetcher - %s '
...
class MSocialServices(mongo.Document):
    ...
    def follow_twitter_friends(self):
        ...
        # Follow any friends already on PyTune
        ...
        # Friends already on PyTune should follow back
    ...
    def post_to_facebook(self, shared_story):
        ...
        try:
            ...
            api.put_object('me', '%s:share' % settings.FACEBOOK_NAMESPACE, 
                           link=shared_story.pytuneblog_permalink(), 
                           type="link", 
                           name=shared_story.decoded_story_title, 
                           description=content, 
                           website=shared_story.pytuneblog_permalink(),
                           message=message,
                           )
    ...
    def post_to_appdotnet(self, shared_story):
        ...
        try:
            ...
            api.createPost(text=message, links=[{
                ...
                'url': shared_story.pytuneblog_permalink()
            }])
        except Exception, e:
            print e
            return
