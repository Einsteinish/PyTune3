PYTUNE.ReaderIntro = function(options) {
    var defaults = {
        modal_container_class: "NB-full-container"
    };
    var intro_page = PYTUNE.assets.preference('intro_page');
...
PYTUNE.ReaderIntro.prototype = new PYTUNE.Modal;
PYTUNE.ReaderIntro.prototype.constructor = PYTUNE.ReaderIntro;

_.extend(PYTUNE.ReaderIntro.prototype, {
    ...
    make_modal: function() {
        ...
        this.$modal = $.make('div', { className: 'NB-modal-intro NB-modal' }, [
            ...
            $.make('img', { className: 'NB-intro-spinning-logo', src: PYTUNE.Globals.MEDIA_URL + 'img/logo_512.png' }),
            ...
            $.make('div', { className: 'NB-page NB-page-4' }, [
                $.make('h4', { className: 'NB-page-4-started' }, "Keep up-to-date with PyTune"),
                $.make('table', { className: 'NB-intro-follows', cellpadding: 0, cellspacing: 0 }, [
                    $.make('tr', [
                        $.make('td', { className: 'NB-intro-uptodate-follow NB-intro-uptodate-follow-twitter' }, [
                            $.make('input', { type: 'checkbox', id: 'NB-intro-uptodate-follow-khong' }),
                            $.make('label', { 'for': 'NB-intro-uptodate-follow-khong' }, [
                                $.make('img', { src: "https://pbs.twimg.com/profile_images/731993784171397120/SKpL9D5p.jpg", style: 'border-color: #505050;' }),
                                $.make('span', '@KHong')
                            ]),
                            $.make('iframe', { allowtransparency: "true", frameborder: "0", scrolling: "no", src: "//platform.twitter.com/widgets/follow_button.html?screen_name=khong", width: 260, height: 20 })
                        ]),
                        $.make('td', { className: 'NB-intro-uptodate-follow NB-intro-uptodate-follow-twitter' }, [
                            $.make('input', { type: 'checkbox', id: 'NB-intro-uptodate-follow-pytune' }),
                            $.make('label', { 'for': 'NB-intro-uptodate-follow-pytune' }, [
                                $.make('img', { src: PYTUNE.Globals.MEDIA_URL + '/img/logo_128.png', style: 'border-color: #505050;' }),
                                $.make('span', '@pytune')
                            ]),
                            $.make('iframe', { allowtransparency: "true", frameborder: "0", scrolling: "no", src: "//platform.twitter.com/widgets/follow_button.html?screen_name=pytune", width: 260, height: 20 })
                        ])
                    ]),
                ...
                $.make('div', { className: 'NB-intro-section' }, [
                    "You're ready to go! Hope you enjoy PyTun."
                ])
            ]),
    ...
    fetch_categories: function(callback) {
        ...
        PYTUNE.assets.fetch_categories(_.bind(function(data) {
    ...
    make_categories: function() {
        ...
        var categories = _.map(this.categories, _.bind(function(category) {
            var $feeds = _.compact(_.map(category.feed_ids, _.bind(function(feed_id) {
                ...
                if (!feed) return;
                feed = new PYTUNE.Models.Feed(feed);
    ...
    toggle_category: function(category, $category) {
        ...
        if (this.chosen_categories.length) {
            PYTUNE.assets.preference('has_setup_feeds', true);
    ...
    submit_categories: function() {
        if (this.chosen_categories.length) {
            PYTUNE.assets.subscribe_to_categories(this.chosen_categories, function() {
    ...
    fetch_friends: function(callback) {
        ...
        PYTUNE.assets.fetch_friends(_.bind(function(data) {
            this.profile = PYTUNE.assets.user_profile;
            ...
            if (this.services && this.services[service][service+'_uid'] && !this.services[service].syncing) {
                $service = $.make('div', { className: 'NB-intro-module-container NB-friends-service NB-connected NB-friends-service-'+service }, [
                    ...
                    $.make('div', { className: 'NB-intro-module NB-intro-module-'+service }, [
                        $.make('h3', [
                            $.make('img', { src: PYTUNE.Globals.MEDIA_URL + '/img/reader/' + service + '_big.png', width: 44, height: 44 })
                        ]),
    ...
    disconnect: function(service) {
        ...
        PYTUNE.assets.disconnect_social_service(service, _.bind(function(data) {
            this.services = data.services;
            this.make_find_friends_and_services();
    ...
    page: function(page_number, from_page) {
      ...
      clearTimeout(this.sync_interval);
      PYTUNE.assets.preference('intro_page', page_number);
    ...
    count_feeds: function(options) {
        options = options || {};
        var feed_count = options.fake_count || PYTUNE.assets.feeds.size();
        var starred_count = options.starred_count || PYTUNE.assets.starred_count;
    ...
    close_and_load_pytune_blog: function() {
      this.close();
      PYTUNE.reader.load_feed_in_tryfeed_view(this.pytune_feed.id, {'feed': this.pytune_feed});
    },
    ...
    google_reader_connect: function(options) {
        ...
        PYTUNE.reader.flags.importing_from_google_reader = true;
    },
    ...
    start_import_from_google_reader: function(data) {
        ...
        if (data && data.error) {
            ...
            $error.hide();
            PYTUNE.reader.flags.importing_from_google_reader = false;
            this.advance_import_carousel(1, data);
            $loading.addClass('NB-active');
            if (data && data.starred_only) {
                    PYTUNE.assets.start_import_starred_stories_from_google_reader(
                        _.bind(this.finish_import_from_google_reader, this));                
            } else {
                    PYTUNE.assets.start_import_from_google_reader(
                        _.bind(this.finish_import_from_google_reader, this));
            }
        }
    },
    ...
    finish_import_from_google_reader: function(data) {
        ...
        PYTUNE.assets.load_feeds(_.bind(function() {
            $loading.removeClass('NB-active');
            this.advance_import_carousel(2);
            if (data.delayed) {
                PYTUNE.reader.flags.delayed_import = true;
    ...
    handle_opml_upload: function() {
        ...
        // PYTUNE.log(['Uploading']);
        var params = {
            url: PYTUNE.URLs['opml-upload'],
            type: 'POST',
            dataType: 'json',
            success: function (data, status) {
                PYTUNE.assets.load_feeds(function() {
                ...
                PYTUNE.reader.load_recommended_feed();
            },
            error: function (data, status, e) {
                self.advance_import_carousel(2);
                $loading.removeClass('NB-active');
                PYTUNE.log(['Error', data, status, e]);
    ...
    follow_twitter_account: function(username) {
        ...
        if ($input.is(':checked')) {
            ...
            if (this.services.twitter.twitter_uid) {
                PYTUNE.assets.follow_twitter_account(username);
            } else {
            ...
            if (this.services.twitter.twitter_uid) {
                PYTUNE.assets.unfollow_twitter_account(username);
            }
        }
    },
    ...
    handle_click: function(elem, e) {
        ...
        $.targetIs(e, { tagSelector: '.NB-tutorial-finish-pytune-blog' }, function($t, $p) {
            e.preventDefault();
            
            self.close_and_load_pytune_blog();
        });
    ...
    handle_change: function(elem, e) {
        ...
        $.targetIs(e, { tagSelector: '.NB-friends-autofollow-checkbox' }, function($t, $p) {
            PYTUNE.assets.preference('autofollow_friends', $t.is(':checked'));
        });
        $.targetIs(e, { tagSelector: '#NB-intro-uptodate-follow-pytune' }, function($t, $p) {
            self.follow_twitter_account('pytune');
        });
        $.targetIs(e, { tagSelector: '#NB-intro-uptodate-follow-khong' }, function($t, $p) {
            self.follow_twitter_account('khong');
        });
    }
