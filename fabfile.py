...
env.PYTUNE_PATH = "/srv/pytune"
env.SECRETS_PATH = "/srv/secrets-pytune"
env.VENDOR_PATH   = "/srv/code"
env.user = 'sfvue'
env.key_filename = os.path.join(env.SECRETS_PATH, 'keys/pytune.key')
...
try:
    ...
    env.roledefs = {
        'app'   : ['app01.pytune.com'],
        'db'    : ['db01.pytune.com'],
        'task'  : ['task01.pytune.com'],
    }
...
def ec2():
    env.user = 'ubuntu'
    env.key_filename = ['/Users/sfvue/.ec2/sfvue.pem']
    assign_digitalocean_roledefs()
...
def setup_user():
    # run('useradd -c "PyTune" -m pytune -s /bin/zsh')
    ...
    run("echo \"\n\" >> ~sfvue/.ssh/authorized_keys")
    run('echo `cat authorized_keys` >> ~sfvue/.ssh/authorized_keys')
    run('rm authorized_keys')

def copy_ssh_keys():
    put(os.path.join(env.SECRETS_PATH, 'keys/pytune.key.pub'), "local_keys")
    run("echo \"\n\" >> ~sfvue/.ssh/authorized_keys")
    run("echo `cat local_keys` >> ~sfvue/.ssh/authorized_keys")
    run("rm local_keys")
...
def setup_repo():
    ...
    with settings(warn_only=True):
        run('git clone https://github.com/Einsteinish/PyTune.git %s' % env.PYTUNE_PATH)
    with settings(warn_only=True):
        sudo('ln -sfn /srv/code /home/%s/code' % env.user)
        sudo('ln -sfn /srv/pytune /home/%s/pytune' % env.user)

def setup_repo_local_settings():
    with virtualenv():
        run('cp local_settings.py.template local_settings.py')
        run('mkdir -p logs')
        run('touch logs/pytune.log')
...
#     #      os.path.join(env.PYTUNE_PATH, 'config/requirements.txt'))
#     pip()
#     put('config/pystartup.py', '.pystartup')
#
#     # with cd(os.path.join(env.PYTUNE_PATH, 'vendor/cjson')):
...
def setup_virtualenv():
    ...
    with prefix('WORKON_HOME=%s' % os.path.join(env.PYTUNE_PATH, 'venv')):
        with prefix('source /usr/local/bin/virtualenvwrapper.sh'):
            with cd(env.PYTUNE_PATH):
                # sudo('rmvirtualenv pytune')
                # sudo('rm -fr venv')
                with settings(warn_only=True):
                    run('mkvirtualenv --no-site-packages pytune')
                run('echo "import sys; sys.setdefaultencoding(\'utf-8\')" | sudo tee venv/pytune/lib/python2.7/sitecustomize.py')
...
def setup_forked_mongoengine():
    with cd(os.path.join(env.VENDOR_PATH, 'mongoengine')), settings(warn_only=True):
        run('git remote add clay https://github.com/Einsteinish/mongoengine.git')
        run('git pull')
        run('git fetch clay')
...
def setup_logrotate(clear=True):
    if clear:
        run('find /srv/pytune/logs/*.log | xargs tee')
    put('config/logrotate.conf', '/etc/logrotate.d/pytune', use_sudo=True)
    ...
    sudo('chown root.root /etc/logrotate.d/{pytune,mongodb,nginx}')
    sudo('chmod 644 /etc/logrotate.d/{pytune,mongodb,nginx}')
    with settings(warn_only=True):
        sudo('chown sfvue.sfvue /srv/pytune/logs/*.log')
    sudo('logrotate -f /etc/logrotate.d/pytune')
...
def setup_sudoers(user=None):
    sudo('echo "%s ALL=(ALL) NOPASSWD: ALL" | sudo tee /etc/sudoers.d/sfvue' % (user or env.user))
    sudo('chmod 0440 /etc/sudoers.d/sfvue')
...
def config_nginx():
    ...
    put("config/nginx.pytune.conf", "/usr/local/nginx/conf/sites-enabled/pytune.conf", use_sudo=True)
    put("config/nginx-init", "/etc/init.d/nginx", use_sudo=True)
    sudo('sed -i -e s/nginx_none/`cat /etc/hostname`/g /usr/local/nginx/conf/sites-enabled/pytune.conf')
...
def setup_staging():
    run('git clone https://github.com/Einsteinish/PyTune.git staging')
    with cd('~/staging'):
        run('cp ../pytune/local_settings.py local_settings.py')
        run('mkdir -p logs')
        run('touch logs/pytune.log')
...
def copy_app_settings():
    put(os.path.join(env.SECRETS_PATH, 'settings/app_settings.py'), 
        '%s/local_settings.py' % env.PYTUNE_PATH)
    run('echo "\nSERVER_NAME = \\\\"`hostname`\\\\"" >> %s/local_settings.py' % env.PYTUNE_PATH)

def assemble_certificates():
    with lcd(os.path.join(env.SECRETS_PATH, 'certificates/comodo')):
        local('pwd')
        local('cat STAR_pytune_com.crt EssentialSSLCA_2.crt ComodoUTNSGCCA.crt UTNAddTrustSGCCA.crt AddTrustExternalCARoot.crt > pytune.com.crt')
        
def copy_certificates():
    cert_path = '%s/config/certificates' % env.PYTUNE_PATH
    run('mkdir -p %s' % cert_path)
    put(os.path.join(env.SECRETS_PATH, 'certificates/pytune.com.crt'), cert_path)
    put(os.path.join(env.SECRETS_PATH, 'certificates/pytune.com.key'), cert_path)
    put(os.path.join(env.SECRETS_PATH, 'certificates/comodo/pytune.com.pem'), cert_path)
    run('cat %s/pytune.com.pem > %s/pytune.pem' % (cert_path, cert_path))
    run('cat %s/pytune.com.key >> %s/pytune.pem' % (cert_path, cert_path))

@parallel
def maintenance_on():
    put('templates/maintenance_off.html', '%s/templates/maintenance_off.html' % env.PYTUNE_PATH)
    with virtualenv():
...
def setup_haproxy(debug=False):
    ...
    cert_path = "%s/config/certificates" % env.PYTUNE_PATH
    run('cat %s/pytune.com.crt > %s/pytune.pem' % (cert_path, cert_path))
    run('cat %s/pytune.com.key >> %s/pytune.pem' % (cert_path, cert_path))
...
def setup_rabbitmq():
    ...
    sudo('rabbitmqctl add_user pytune pytune')
    sudo('rabbitmqctl add_vhost pytunevhost')
    sudo('rabbitmqctl set_permissions -p pytunevhost pytune ".*" ".*" ".*"')
...
def copy_munin_data(from_server):
    put(os.path.join(env.SECRETS_PATH, 'keys/pytune.key'), '~/.ssh/pytune.key')
    put(os.path.join(env.SECRETS_PATH, 'keys/pytune.key.pub'), '~/.ssh/pytune.key.pub')
    run('chmod 600 ~/.ssh/pytune*')

    put("config/munin.nginx.conf", "/usr/local/nginx/conf/sites-enabled/munin.conf", use_sudo=True)
    sudo('/etc/init.d/nginx reload')

    run("rsync -az -e \"ssh -i /home/sfvue/.ssh/pytune.key\" --stats --progress %s:/var/lib/munin/ /srv/munin" % from_server)
    sudo("mv /var/lib/munin /var/lib/bak-munin")
    sudo("mv /srv/munin /var/lib/")
    sudo("chown munin.munin -R /var/lib/munin")

    # run("rsync -az -e \"ssh -i /home/sfvue/.ssh/pytune.key\" --stats --progress %s:/etc/munin/ /srv/munin-etc" % from_server)
...
def setup_db_munin():
    sudo('cp -frs %s/config/munin/mongo* /etc/munin/plugins/' % env.PYTUNE_PATH)
    sudo('cp -frs %s/config/munin/pg_* /etc/munin/plugins/' % env.PYTUNE_PATH)
    sudo('cp -frs %s/config/munin/redis_* /etc/munin/plugins/' % env.PYTUNE_PATH)
    with cd(env.VENDOR_PATH), settings(warn_only=True):
...
def setup_usage_monitor():
    sudo('ln -fs %s/utils/monitor_disk_usage.py /etc/cron.daily/monitor_disk_usage' % env.PYTUNE_PATH)
    sudo('/etc/cron.daily/monitor_disk_usage')
    
@parallel
def setup_redis_monitor():
    sudo('ln -fs %s/utils/monitor_redis_bgsave.py /etc/cron.daily/monitor_redis_bgsave' % env.PYTUNE_PATH)
...
def setup_motd(role='app'):
    motd = '/etc/update-motd.d/22-pytune-motd'
...
def copy_task_settings():
    ...
    with settings(warn_only=True):
        put(os.path.join(env.SECRETS_PATH, 'settings/task_settings.py'), 
            '%s/local_settings.py' % env.PYTUNE_PATH)
        run('echo "\nSERVER_NAME = \\\\"%s\\\\"" >> %s/local_settings.py' % (host, env.PYTUNE_PATH))

@parallel
def copy_spam():
    put(os.path.join(env.SECRETS_PATH, 'spam/spam.py'), '%s/apps/social/spam.py' % env.PYTUNE_PATH)
...
def deploy_code(copy_assets=False, rebuild=False, fast=False, reload=False):
    ...
    with virtualenv(), settings(warn_only=True):
        if reload:
            ...
            sudo('kill -HUP `cat /srv/pytune/logs/gunicorn.pid`')
...
def staging():
    with cd('~/staging'):
        ...
        run('curl -s http://dev.pytune.com > /dev/null')
        run('curl -s http://dev.pytune.com/m/ > /dev/null')
...
def compress_assets(bundle=False):
    local('jammit -c assets.yml --base-url http://www.pytune.com --output static')
    ...
    while True:
        try:
            success = False
            with settings(warn_only=True):
                local('PYTHONPATH=/srv/pytune python utils/backups/s3.py set static.tgz')
                success = True
            if not success:
                raise Exception("Ack!")
            break
...
def transfer_assets():
    ...
    # run('PYTHONPATH=/srv/pytune python s3.py get deploy_%s.tgz' % filename)
    run('PYTHONPATH=/srv/pytune python utils/backups/s3.py get static.tgz')
...
def setup_redis_backups(name=None):
    ...
    crontab = ("0 4 * * * python /srv/pytune/utils/backups/backup_redis%s.py" % 
...
def setup_mongo_backups():
    ...
    crontab = "0 4 * * * python /srv/pytune/utils/backups/backup_mongo.py"
    run('(crontab -l ; echo "%s") | sort - | uniq - | crontab -' % crontab)
    run('crontab -l')
...
def setup_postgres_backups():
    ...
    crontab = """
0 4 * * * python /srv/pytune/utils/backups/backup_psql.py
...
def backup_redis(name=None):
    run('python /srv/pytune/utils/backups/backup_redis%s.py' % (("_%s"%name) if name else ""))
    
def backup_mongo():
    run('python /srv/pytune/utils/backups/backup_mongo.py')

def backup_postgresql():
    run('python /srv/pytune/utils/backups/backup_psql.py')
...
def setup_time_calibration():
    ...
    put('config/ntpdate.cron', '%s/' % env.PYTUNE_PATH)
    sudo('chown root.root %s/ntpdate.cron' % env.PYTUNE_PATH)
    sudo('chmod 755 %s/ntpdate.cron' % env.PYTUNE_PATH)
    sudo('mv %s/ntpdate.cron /etc/cron.hourly/ntpdate' % env.PYTUNE_PATH)
    with settings(warn_only=True):
...
def restore_postgres(port=5433):
    ...
    yes = prompt("Dropping and creating PyTune PGSQL db. Sure?")
    if yes != 'y':
        return
    # run('PYTHONPATH=%s python utils/backups/s3.py get backup_postgresql_%s.sql.gz' % (env.PYTUNE_PATH, backup_date))
    # sudo('su postgres -c "createuser -p %s -U pytune"' % (port,))
    run('dropdb pytune -p %s -U postgres' % (port,), pty=False)
    run('createdb pytune -p %s -O pytune' % (port,), pty=False)
    run('pg_restore -p %s --role=pytune --dbname=pytune /Users/sfvue/Documents/backups/backup_postgresql_%s.sql.gz' % (port, backup_date), pty=False)

def restore_mongo():
    backup_date = '2012-07-24-09-00'
    run('PYTHONPATH=/srv/pytune python s3.py get backup_mongo_%s.tgz' % (backup_date))
