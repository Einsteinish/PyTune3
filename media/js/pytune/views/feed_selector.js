PYTUNE.Views.FeedSelector = Backbone.View.extend({
    ...
    initialize: function() {
        this.selected_feeds = new PYTUNE.Collections.Feeds();
    },
    ...
    show_feed_selector: function() {
        ...
        var $feed_list    = PYTUNE.reader.$s.$feed_list;
        var $social_feeds = PYTUNE.reader.$s.$social_feeds;
        var $body = PYTUNE.reader.$s.$body;

        this.$el.show();
        $input.val('');
        $input.focus();
        PYTUNE.app.feed_list.options.feed_chooser = true;
        ...
        PYTUNE.reader.layout.leftLayout.sizePane('north');
        
        if (PYTUNE.reader.flags['sidebar_closed']) {
            PYTUNE.reader.layout.outerLayout.show('west', true);
        }
    },
    ...
    hide_feed_selector: function() {
        ...
        this.$el.hide();
        this.$next_feed = null;
        PYTUNE.app.feed_list.options.feed_chooser = false;
        PYTUNE.assets.feeds.trigger('reset');
        ...
        if (PYTUNE.reader.flags['sidebar_closed']) {
            PYTUNE.reader.layout.outerLayout.hide('west');
        }
    },
    ...
    filter_feed_selector: function(e) {
        ...
        var feeds = PYTUNE.assets.feeds.filter(function(feed){ 
            return _.string.contains(feed.get('feed_title') && feed.get('feed_title').toLowerCase(), input) || feed.id == input;
        });
        var socialsubs = PYTUNE.assets.social_feeds.filter(function(feed){ 
    ...
    select: function(direction) {
        ...
        this.$next_feed = PYTUNE.reader.get_next_feed(direction, $current_feed);
        
        $('.NB-feed-selector-selected').removeClass('NB-feed-selector-selected');
        this.$next_feed.addClass('NB-feed-selector-selected');
        PYTUNE.app.feed_list.scroll_to_show_highlighted_feed();
    },
    
    open: function(e) {
        var feed_id = this.$next_feed.data('id');
        if (_.string.include(feed_id, 'social:')) {
            PYTUNE.reader.open_social_stories(this.$next_feed.data('id'), {
                $feed: this.$next_feed
            });
        } else {
            PYTUNE.reader.open_feed(this.$next_feed.data('id'), {$feed: this.$next_feed});
        }
