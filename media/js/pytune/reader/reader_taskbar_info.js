PYTUNE.Views.ReaderTaskbarInfo = Backbone.View.extend({
    ...
    render: function() {
        PYTUNE.reader.$s.$story_taskbar.append(this.$el);
        return this;
    },
    ...
    show_stories_progress_bar: function(feeds_loading, message) {
        ...
        else unreads = PYTUNE.reader.get_total_unread_count(false) / 10;
        PYTUNE.reader.animate_progress_bar($bar, unreads / 10);
    ...
    show_stories_error: function(data, message) {
        PYTUNE.log(["show_stories_error", data]);
        this.hide_stories_progress_bar();
        
        PYTUNE.app.original_tab_view.iframe_not_busting();
        ...
        if (data && data.status) {
            if (data.status == 502) {
                message = "PyTune is down right now. <br> Try again soon.";
            } else if (data.status == 503) {
                message = "PyTune is in maintenance mode. <br> Try again soon.";
                this.show_maintenance_page();
            }
            PYTUNE.assets.flags['no_more_stories'] = true;
            PYTUNE.app.story_titles.end_loading();
            PYTUNE.app.story_list.end_loading();
        }
    ...
    show_maintenance_page: function() {
        PYTUNE.reader.switch_taskbar_view('page', {skip_save_type: 'maintenance'});
    }
