PYTUNE.Views.SidebarHeader = Backbone.View.extend({
    ...
    render: function() {
        ...
        var hide_read_feeds = PYTUNE.assets.preference('hide_read_feeds');
        // PYTUNE.log(["render feed list header", this.feed_collection.length, this.feeds_count, hide_read_feeds]);
    ...
    toggle_hide_read_preference: function() {
        var hide_read_feeds = PYTUNE.assets.preference('hide_read_feeds');
        if (PYTUNE.reader.flags['feed_list_showing_starred']) hide_read_feeds = true;
    ...
    count: function() {
        this.unread_counts = PYTUNE.assets.folders.unread_counts();
        this.unread_counts = PYTUNE.assets.social_feeds.unread_counts(this.unread_counts);
        
        if (!PYTUNE.Globals.is_authenticated) return;
        if (!PYTUNE.assets.preference('title_counts')) return;
        
        var counts = [];
        var unread_view = _.isNumber(this.options.unread_view) && this.options.unread_view || PYTUNE.assets.preference('unread_view');
        if (unread_view <= -1) {
    ...
    show_interactions_popover: function() {
        PYTUNE.InteractionsPopover.create({});
    }
