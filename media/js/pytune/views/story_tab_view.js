PYTUNE.Views.StoryTabView = Backbone.View.extend({
        
    flags: {},
    
    initialize: function() {
        this.setElement(PYTUNE.reader.$s.$story_view);
        this.$iframe = PYTUNE.reader.$s.$story_iframe;
    ...
    prepare_story: function(story, is_temporary) {
        ...
        if ((feed && feed.get('disabled_page')) || 
            PYTUNE.utils.is_url_iframe_buster(story.get('story_permalink'))) {
            if (!is_temporary) {
                PYTUNE.reader.switch_taskbar_view('text', {skip_save_type: 'story'});
                PYTUNE.app.taskbar_info.show_stories_error({}, "Sorry, the original story<br />could not be proxied.");
            }
        } else {
            PYTUNE.reader.switch_taskbar_view('story', {skip_save_type: is_temporary ? 'story' : false});
        }
    },
    ...
    show_loading: function() {
        PYTUNE.app.taskbar_info.hide_stories_error();
        PYTUNE.app.taskbar_info.show_stories_progress_bar(10, "Fetching story");
    },
    ...
    select_story: function(story, selected) {
        if (selected && PYTUNE.reader.story_view == 'story') {
