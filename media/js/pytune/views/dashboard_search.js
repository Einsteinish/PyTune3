PYTUNE.Views.DashboardSearch = Backbone.View.extend({
    ...
    search_sites: function() {
        ...
        PYTUNE.assets.search_for_feeds(query, _.bind(function(data) {
            ...
            if (!data || !data.feeds || !data.feeds.length) {
                ...
                this.$results.html($.make('div', _.map(data.feeds, function(feed) {
                    var model = new PYTUNE.Models.Feed(feed);
                    return new PYTUNE.Views.FeedBadge({model: model});
                })));
            }
    ...
    add_url: function() {
        ...
        PYTUNE.reader.open_add_feed_modal({url: query});
    }
