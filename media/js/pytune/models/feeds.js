PYTUNE.Models.Feed = Backbone.Model.extend({
    ...
    on_change: function() {
        if (!('selected' in this.changedAttributes())) {
            PYTUNE.log(['Feed Change', this.changedAttributes(), this.previousAttributes()]);
        }
    },
    ...
    change_counts: function(data, count, options) {
        ...
        if (this.get('selected') && options.refresh_feeds) {
            ...
            PYTUNE.reader.feed_unread_count(this.id);
        }
    },
    ...
    delete_feed: function(options) {
        ...
        PYTUNE.assets.delete_feed(this.id, view.options.folder_title, function() {
    ...
    move_to_folder: function(to_folder, options) {
        ...
        PYTUNE.reader.flags['reloading_feeds'] = true;
        PYTUNE.assets.move_feed_to_folder(this.id, in_folder, to_folder, function() {
            PYTUNE.reader.flags['reloading_feeds'] = false;
            _.delay(function() {
    ...
    move_to_folders: function(to_folders, options) {
        ...
        PYTUNE.reader.flags['reloading_feeds'] = true;
        PYTUNE.assets.move_feed_to_folders(this.id, in_folders, to_folders, function() {
            PYTUNE.reader.flags['reloading_feeds'] = false;
            _.delay(function() {
                PYTUNE.reader.$s.$feed_list.css('opacity', 1).animate({'opacity': 0}, {
                    'duration': 100, 
    ...
    rename: function(new_title) {
        ...
        PYTUNE.assets.rename_feed(this.id, new_title);
    },
    ...
    unread_counts: function() {
        var starred_feed = PYTUNE.assets.starred_feeds.get_feed(this.id);
    ...
    has_unreads: function(options) {
        options = options || {};
        var unread_view = PYTUNE.assets.preference('unread_view');
...
PYTUNE.Collections.Feeds = Backbone.Collection.extend({
    
    model: PYTUNE.Models.Feed,
