PYTUNE.Views.FeedBadge = Backbone.View.extend({
    ...
    render: function() {
        var subscribed = PYTUNE.assets.get_feed(this.model.id);
    ...
    try_feed: function() {
        PYTUNE.reader.load_feed_in_tryfeed_view(this.model.id);
    },
    
    add_feed: function() {
        PYTUNE.reader.open_add_feed_modal({url: this.model.get('feed_address')});
    },
    
    open_stats: function() {
        PYTUNE.assets.load_canonical_feed(this.model.id, _.bind(function() {
            PYTUNE.reader.open_feed_statistics_modal(this.model.id);
        }, this));
