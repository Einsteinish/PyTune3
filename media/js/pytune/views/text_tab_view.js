PYTUNE.Views.TextTabView = Backbone.View.extend({
    ...
    fetch_and_render: function(story, is_temporary) {
        ...
        if (!story && is_temporary) {
            PYTUNE.reader.show_next_story(1);
            story = PYTUNE.reader.active_story;
        }
        if (!story) return;

        if (is_temporary && _.contains(['split', 'full'], PYTUNE.assets.view_setting(PYTUNE.reader.active_feed, 'layout'))) {
            PYTUNE.reader.switch_taskbar_view('text', {
        ...
        this.show_loading();
        PYTUNE.assets.fetch_original_text(story.get('id'), story.get('story_feed_id'), 
    ...
    render: function(data) {
        ...
        if (!this.story.get('original_text') || 
            ...
            PYTUNE.reader.make_story_titles_pane_counter();
    ...
    show_loading: function() {
        ...
        PYTUNE.app.taskbar_info.show_stories_progress_bar(10, "Fetching text");
    },
    ...
    error: function() {
        this.hide_loading();
        PYTUNE.app.taskbar_info.show_stories_error({}, "Sorry, the story\'s text<br />could not be extracted.");
