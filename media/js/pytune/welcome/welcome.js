PYTUNE.Welcome = Backbone.View.extend({
    ...
    show_signin_form: function() {
        var open = !PYTUNE.reader.flags['sidebar_closed'];
        this.hide_tryout();
    ...
    show_tryout: function() {
        if (!PYTUNE.reader) return;
        
        if (!this.flags.loaded) {
            PYTUNE.reader.$s.$layout.layout().hide('west', true);
            PYTUNE.reader.$s.$layout.show();
            this.flags.loaded = true;
        }
        var open = PYTUNE.reader.toggle_sidebar();
