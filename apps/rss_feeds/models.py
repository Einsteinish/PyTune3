...
class Feed(models.Model):
    ...
    @property
    def permalink(self):
        return "%s/site/%s/%s" % (settings.PYTUNE_URL, self.pk, slugify(self.feed_title.lower()[:50]))
    ...
    def update(self, **kwargs):
        ...
        if getattr(settings, 'TEST_DEBUG', False):
            ...
            self.feed_address = self.feed_address.replace("%(PYTUNE_DIR)s", settings.PYTUNE_DIR)
            if self.feed_link:
                self.feed_link = self.feed_link.replace("%(PYTUNE_DIR)s", settings.PYTUNE_DIR)
    ...
    def format_story(cls, story_db, feed_id=None, text=False, include_permalinks=False,
        ...
        if include_permalinks and hasattr(story_db, 'pytuneblog_permalink'):
            story['pytuneblog_permalink'] = story_db.pytuneblog_permalink()
        if text:
...
class MStarredStory(mongo.Document):
    ...
    def trim_old_stories(cls, stories=10, days=90, dryrun=False):
        ...
        stats = settings.MONGODB.pytune.starred_stories.aggregate([{
            "$group": {
                "_id":      "$user_id",
                "stories":  {"$sum": 1},
            },
...
class MStarredStoryCounts(mongo.Document):
    ...
    def rss_url(self, secret_token=None):
        ...
        return "%s/reader/starred_rss/%s/%s/%s" % (settings.PYTUNE_URL, self.user_id, 
                                                   secret_token, slug)
