...
def load_social_stories(request, user_id, username=None):
    ...
    if social_profile.private and not social_profile.is_followed_by_user(user.pk):
        message = "%s has a private pytuneblog and you must be following them in order to read it." % social_profile.username
    elif query:
...
@json.json_view
def load_river_pytuneblog(request):
    limit             = 10
    start             = time.time()
    user              = get_user(request)
    ...
    logging.user(request, "~FYLoading ~FCriver ~FMpytuneblogs~FC stories~FY: ~SBp%s~SN (%s/%s "
...
def load_social_page(request, user_id, username=None, **kwargs):
    ...
    if '/story/' in path and format != 'html':
        ...
        if active_story_db:
            ...
            active_story['pytuneblog_permalink'] = active_story_db.pytuneblog_permalink()
...
def load_user_profile(request):
    ...
    logging.user(request, "~BB~FRLoading social profile and pytuneblog settings")
...
def find_friends(request):
    ...
    if not profiles:
        profiles = MSocialProfile.objects.filter(pytuneblog_title__icontains=query)[:limit]
    if not profiles:
...
def shared_stories_rss_feed(request, user_id, username):
    ...
    data['link'] = social_profile.pytuneblog_url
    data['description'] = "Stories shared by %s on PyTune." % user.username
