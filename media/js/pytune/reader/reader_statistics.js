PYTUNE.ReaderStatistics = function(feed_id, options) {
    var defaults = {};
...
PYTUNE.ReaderStatistics.prototype = new PYTUNE.Modal;
PYTUNE.ReaderStatistics.prototype.constructor = PYTUNE.ReaderStatistics;

_.extend(PYTUNE.ReaderStatistics.prototype, {
    ...
    make_stats: function(data) {
        var update_interval = PYTUNE.utils.calculate_update_interval(data['update_interval_minutes']);
        var premium_update_interval = PYTUNE.utils.calculate_update_interval(data['premium_update_interval_minutes']);
        ...
        var $stats = $.make('div', { className: 'NB-modal-statistics-info' }, [
            (!this.options.social_feed && $.make('div', { className: 'NB-statistics-stat NB-statistics-updates'}, [
              ...
              $.make('div', { className: 'NB-statistics-update'}, [
                (data['push'] && $.make('div', { className: 'NB-statistics-realtime' }, [
                    $.make('div', { className: 'NB-statistics-label' }, [
                        $.make('img', { src: PYTUNE.Globals.MEDIA_URL + '/img/reader/realtime_spinner.gif', className: 'NB-statisics-realtime-spinner' }),
                        'Real-time'
                    ]),
              ...
              (!PYTUNE.Globals.is_premium && $.make('div', { className: 'NB-statistics-premium-stats' }, [
                  $.make('div', { className: 'NB-statistics-update'}, [
                    $.make('div', { className: 'NB-statistics-label' }, [
                        'If you went ',
    ...
    make_chart_count: function(data) {
        var labels = _.map(data['story_count_history'], function(date) {
            ...
            return PYTUNE.utils.shortMonthNames[date.getMonth()] + " " + date.getUTCFullYear();
        });
        if (labels.length > 16) {
    ...
    make_chart_days: function(data) {
        var labels = PYTUNE.utils.dayNames;
