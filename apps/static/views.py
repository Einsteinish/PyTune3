...
def ios_plist(request):
    filename = os.path.join(settings.PYTUNE_DIR, 'clients/ios/PyTune.plist')
    manifest = open(filename).read()
    
    logging.user(request, "~SK~FR~BBDownloading PyTune.plist...")
    return HttpResponse(manifest, content_type='text/xml')
    
def ios_ipa(request):
    filename = os.path.join(settings.PYTUNE_DIR, 'clients/ios/PyTune.ipa')
    manifest = open(filename).read()
    
    logging.user(request, "~SK~FR~BBDownloading PyTune.ipa...")
