PYTUNE.StoryOptionsPopover = PYTUNE.ReaderPopover.extend({
    ...
    close: function() {
        PYTUNE.reader.$s.$taskbar_options.removeClass('NB-active');
            ...
            $.make('div', { className: 'NB-popover-section' }, [
                ...
                $.make('ul', { className: 'segmented-control-vertical NB-options-font-family' }, [
                    ...
                    $.make('li', { className: 'NB-font-family-option NB-premium-only NB-options-font-family-gotham' }, [
                        (!PYTUNE.Globals.is_premium && $.make('div', { className: 'NB-tag' }, 'Premium')),
                        'Gotham Narrow'
                    ]),
                ...
                (!PYTUNE.Globals.is_premium && $.make('div', { className: 'NB-premium-explainer' }, [
    ...
    show_correct_options: function() {
        var font_family = PYTUNE.assets.preference('story_styling');
        var story_font_size = PYTUNE.assets.preference('story_size');
        var feed_font_size = PYTUNE.assets.preference('feed_size');
        var line_spacing = PYTUNE.assets.preference('story_line_spacing');
        var titles_layout_pane = PYTUNE.assets.preference('story_pane_anchor');
        var single_story = PYTUNE.assets.preference('feed_view_single_story');
        var grid_columns = PYTUNE.assets.preference('grid_columns');
        ...
        PYTUNE.reader.$s.$taskbar_options.addClass('NB-active');
    ...
    update_font_family: function(setting) {
        PYTUNE.assets.preference('story_styling', setting);
        PYTUNE.reader.apply_story_styling();
    },
    ...
    update_story_font_size: function(setting) {
        PYTUNE.assets.preference('story_size', setting);
        PYTUNE.reader.apply_story_styling();
    },
    
    update_feed_font_size: function(setting) {
        PYTUNE.assets.preference('feed_size', setting);
        PYTUNE.reader.apply_story_styling();
    },
    ...
    update_line_spacing: function(setting) {
        PYTUNE.assets.preference('story_line_spacing', setting);
        PYTUNE.reader.apply_story_styling();
    },
    ...
    update_story_titles_pane: function(setting) {
        var old_anchor = PYTUNE.assets.preference('story_pane_anchor');
        var pane_size = PYTUNE.assets.preference('story_titles_pane_size');
        ...
        PYTUNE.assets.preference('story_pane_anchor', setting);
        PYTUNE.assets.preference('story_titles_pane_size', pane_size);
        PYTUNE.reader.apply_resizable_layout({right_side: true});
    },
    ...
    update_single_story: function(setting) {
        PYTUNE.assets.preference('feed_view_single_story', setting);
        PYTUNE.app.story_list.render();
        _.defer(function() {
            PYTUNE.reader.resize_window();
            if (PYTUNE.reader.active_story) {
                PYTUNE.reader.active_story.set('selected', false).set('selected', true);
            }
        });
    },
    ...
    update_grid_columns: function(setting) {
        PYTUNE.assets.preference('grid_columns', setting);
        PYTUNE.app.story_list.render();
        _.defer(function() {
            PYTUNE.app.story_titles.override_grid();
            PYTUNE.reader.resize_window();
        });
    },
