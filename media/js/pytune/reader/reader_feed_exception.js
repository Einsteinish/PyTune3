PYTUNE.ReaderFeedException = function(feed_id, options) {
    var defaults = {
        'onOpen': function() {
    ...
    this.folder  = this.folder_title && PYTUNE.assets.get_folder(this.folder_title);
    
    this.runner();
};

PYTUNE.ReaderFeedException.prototype = new PYTUNE.Modal;
PYTUNE.ReaderFeedException.prototype.constructor = PYTUNE.ReaderFeedException;

_.extend(PYTUNE.ReaderFeedException.prototype, {
    
    runner: function() {
        if (this.folder) {
            PYTUNE.Modal.prototype.initialize_folder.call(this, this.folder_title);
        } else {
            PYTUNE.Modal.prototype.initialize_feed.call(this, this.feed_id);            
        }
        this.make_modal();
        if (this.feed) {
    ...
    initialize_feed: function(feed_id) {
        ...
        if (this.feed) {
            PYTUNE.Modal.prototype.initialize_feed.call(this, feed_id);
            ...
            PYTUNE.Modal.prototype.initialize_folder.call(this, feed_id);
        }
        ...
        this.$modal = $.make('div', { className: 'NB-modal-exception NB-modal' }, [
            ...
            (this.folder && $.make('div', { className: 'NB-fieldset NB-exception-option NB-exception-option-feed NB-modal-submit' }, [
                ...
                $.make('div', { className: 'NB-fieldset-fields' }, [
                    ...
                    (!PYTUNE.Globals.is_premium && $.make('div', { className: 'NB-premium-only' }, [
    ...
    save_retry_feed: function() {
        ...
        this.model.save_exception_retry(feed_id, function() {
            PYTUNE.reader.force_feed_refresh(feed_id);
            $.modal.close();
        });
    },
    ...
    delete_feed: function() {
        ...
        PYTUNE.reader.manage_menu_delete_feed(feed_id);
    ...
    change_feed_address: function() {
        ...
        if (feed_address.length) {
            this.model.save_exception_change_feed_address(feed_id, feed_address, _.bind(function(data) {
                ...
                PYTUNE.assets.feeds.add(_.values(data.feeds));
                var feed = PYTUNE.assets.get_feed(data.new_feed_id || feed_id);
                var old_feed = PYTUNE.assets.get_feed(feed_id);
                ...
                if (data && data.new_feed_id) {
                    PYTUNE.assets.load_feeds(function() {
                        ...
                        PYTUNE.reader.open_feed(feed.id);
                    });
                }
                
                console.log(["feed address", feed, PYTUNE.assets.get_feed(feed_id)]);
    ...
    change_feed_link: function() {
        ...
        if (feed_link.length) {
            this.model.save_exception_change_feed_link(feed_id, feed_link, _.bind(function(data) {
                ...
                var feed = PYTUNE.assets.get_feed(data.new_feed_id) || PYTUNE.assets.get_feed(feed_id);
    ...
    handle_change: function(elem, e) {
        ...
        $.targetIs(e, { tagSelector: 'input[name=view_settings]' }, function($t, $p){
            if (self.folder) {
                ...
                PYTUNE.assets.view_setting(self.feed_id, {'view': $t.val()});
            }
            self.animate_saved();
        });
        $.targetIs(e, { tagSelector: 'input[name=story_layout]' }, function($t, $p){
            if (self.folder) {
                ...
                PYTUNE.assets.view_setting(self.feed_id, {'layout': $t.val()});
            }
            self.animate_saved();
        });
    }
