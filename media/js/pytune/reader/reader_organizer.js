PYTUNE.ReaderOrganizer = function(user_id, options) {
    var defaults = {
        width: 800,
...
PYTUNE.ReaderOrganizer.prototype = new PYTUNE.Modal;

_.extend(PYTUNE.ReaderOrganizer.prototype, {
    ...
    make_feeds: function(options) {
        ...
        if (this.options.hierarchy == "flat") {
            this.options.folders = new PYTUNE.Collections.Folders(PYTUNE.assets.feeds.pluck('id'));
        } else {
            this.options.folders = PYTUNE.assets.folders;
        }
        this.feed_count = _.unique(this.options.folders.feed_ids_in_folder()).length;
        PYTUNE.Collections.Folders.organizer_sortorder = this.options.sorting;
        PYTUNE.Collections.Folders.organizer_inversesort = this.options.inverse_sorting;
        ...
        PYTUNE.Collections.Folders.organizer_sortorder = null;
        this.options.folders.sort();
    ...
    replace_folders: function() {
        $(".NB-folders", this.$modal).replaceWith(PYTUNE.utils.make_folders());
    },
    ...
    move_feeds: function() {
        ...
        PYTUNE.assets.move_feeds_by_folder(highlighted_feeds, to_folder, new_folder, function() {
    ...
    delete_feeds: function() {
        ...
        PYTUNE.assets.delete_feeds_by_folder(highlighted_feeds, function() {
            PYTUNE.reader.flags['reloading_feeds'] = false;
            $loading.removeClass('NB-active');
            self.reset_feeds();
            $feedlist.replaceWith(self.make_feeds());
