PYTUNE.Views.FeedList = Backbone.View.extend({
    ...
    initialize: function() {
        ...
        PYTUNE.assets.feeds.bind('reset', _.bind(function() {
            this.make_feeds();
        ...
        PYTUNE.assets.social_feeds.bind('reset', _.bind(function() {
            this.make_social_feeds();
        }, this));
        PYTUNE.assets.starred_feeds.bind('reset', _.bind(function(models, options) {
            this.make_starred_tags(options);
        }, this));
        PYTUNE.assets.social_feeds.bind('change:selected', this.scroll_to_selected, this);
        PYTUNE.assets.feeds.bind('change:selected', this.scroll_to_selected, this);
        PYTUNE.assets.starred_feeds.bind('change:selected', this.scroll_to_selected, this);
        if (!PYTUNE.assets.folders.size()) {
            PYTUNE.assets.load_feeds();
        }
        PYTUNE.assets.feeds.bind('add', this.update_dashboard_count, this);
        PYTUNE.assets.feeds.bind('remove', this.update_dashboard_count, this);
    },
    
    make_feeds: function(options) {
        options = options || {};
        var self = this;
        var folders = options.folders || PYTUNE.assets.folders;
        var feeds = PYTUNE.assets.feeds;
        
        this.$el.empty();
        this.$s.$story_taskbar.css({'display': 'block'});
        this.folder_view = new PYTUNE.Views.Folder({
            collection: folders, 
            root: true,
        ...
        if (!this.options.feed_chooser) {
            if (PYTUNE.Globals.is_authenticated && 
                PYTUNE.assets.flags['has_chosen_feeds']) {
                _.delay(function() {
                    if (!PYTUNE.reader.flags['refresh_inline_feed_delay']) return;
                    PYTUNE.reader.start_count_unreads_after_import();
                }, 1000);
                PYTUNE.reader.flags['refresh_inline_feed_delay'] = true;
                PYTUNE.reader.force_feeds_refresh(function() {
                    PYTUNE.reader.flags['refresh_inline_feed_delay'] = false;
                    PYTUNE.reader.finish_count_unreads_after_import();
                }, true, null, function() {
                    PYTUNE.reader.flags['refresh_inline_feed_delay'] = false;
                    PYTUNE.reader.finish_count_unreads_after_import({error: true});
                });
            }
            ...
            if (folders.length) {
                ...
                _.delay(_.bind(PYTUNE.reader.update_starred_count, PYTUNE.reader), 250);
            ...
            if (PYTUNE.reader.flags['showing_feed_in_tryfeed_view'] ||
                PYTUNE.reader.flags['showing_social_feed_in_tryfeed_view']) {
                PYTUNE.reader.hide_tryfeed_view();
                PYTUNE.reader.force_feed_refresh();
            }
        
            _.defer(_.bind(function() {
                PYTUNE.reader.open_dialog_after_feeds_loaded();
                PYTUNE.reader.toggle_focus_in_slider();
                this.scroll_to_selected();
                if (PYTUNE.reader.socket) {
                    PYTUNE.reader.send_socket_active_feeds();
                } else {
                    var force_socket = PYTUNE.Globals.is_admin;
                    PYTUNE.reader.setup_socket_realtime_unread_counts(force_socket);
                }
            }, this));
        }
    ...
    make_social_feeds: function() {
        ...
        var profile = PYTUNE.assets.user_profile;
        var $feeds = PYTUNE.assets.social_feeds.map(function(feed) {
            var feed_view = new PYTUNE.Views.FeedTitleView({
                model: feed, 
                type: 'feed', 
                depth: 0
            }).render();
        ...
        if (PYTUNE.assets.social_feeds.length) {
            $('.NB-feeds-header-river-pytuneblogs-container').css({
                'display': 'block',
                'opacity': 0
            }).animate({'opacity': 1}, {'duration': 700});
        }

        var collapsed = PYTUNE.app.sidebar.check_river_pytuneblog_collapsed({skip_animation: true});
    ...
    make_starred_tags: function(options) {
        ...
        var $feeds = _.compact(PYTUNE.assets.starred_feeds.map(function(feed) {
        ...
        if (PYTUNE.assets.starred_feeds.length) {
    ...
    load_url_next_param: function(route_found) {
        ...
        if (next == 'optout') {
            PYTUNE.reader.open_account_modal({'animate_email': true});
        } else if (next == 'goodies') {
            PYTUNE.reader.open_goodies_modal();
        } else if (next == 'friends') {
            PYTUNE.reader.open_friends_modal();
        } else if (next == 'account') {
            PYTUNE.reader.open_account_modal();
        } else if (next == 'organizer') {
            PYTUNE.reader.open_organizer_modal();
            ...
            PYTUNE.reader.open_account_modal({'change_password': true});
        }

        var url = $.getQueryString('url') || $.getQueryString('add');
        if (url) {
            PYTUNE.reader.open_add_feed_modal({url: url});
        }
    ...
    load_default_folder: function() {
        var default_folder = PYTUNE.assets.preference('default_folder');
        
        if (!default_folder || default_folder == "") {
            PYTUNE.reader.open_river_stories();
        } else {
            var folder = PYTUNE.assets.get_folder(default_folder);
            if (folder) {
                PYTUNE.reader.open_river_stories(folder.folder_view.$el, folder);
            }
        }
    },
    
    update_dashboard_count: function() {
        var feed_count = _.unique(PYTUNE.assets.folders.feed_ids_in_folder()).length;
    ...
    scroll_to_show_selected_feed: function() {
        ...
        var model = PYTUNE.assets.feeds.selected() || 
                    PYTUNE.assets.social_feeds.selected() ||
                    PYTUNE.assets.starred_feeds.selected();
        if (!model) return;
    ...
    scroll_to_show_selected_folder: function() {
        ...
        var folder = PYTUNE.assets.folders.selected();
        if (folder) {
            $selected_view = folder.folder_view.$el;
            $selected_view = $selected_view.find('.folder_title').eq(0);
        }
        
        if (!$selected_view && PYTUNE.reader.active_feed == 'river:') {
            $selected_view = PYTUNE.reader.$s.$river_sites_header.closest(".NB-feeds-header-container");
        } else if (!$selected_view && PYTUNE.reader.active_feed == 'starred') {
            $selected_view = PYTUNE.reader.$s.$starred_header.closest(".NB-feeds-header-container");
        } else if (!$selected_view && PYTUNE.reader.active_feed == 'read') {
            $selected_view = PYTUNE.reader.$s.$read_header.closest(".NB-feeds-header-container");
        }
    ...
    show_read_stories_header: function() {
        PYTUNE.reader.$s.$read_header.closest('.NB-feeds-header-read-container')
                                       .addClass('NB-block');
    }
