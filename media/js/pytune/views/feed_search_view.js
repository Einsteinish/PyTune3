PYTUNE.Views.FeedSearchView = Backbone.View.extend({
    ...
    render: function() {
        if (PYTUNE.app.active_search) {
            PYTUNE.app.active_search.blur_search();
            PYTUNE.app.active_search.remove();
        }
        PYTUNE.app.active_search = this;
        var $view = $(_.template('\
        ...
        ', {
            search: PYTUNE.reader.flags['search']
        }));
    ...
    remove: function() {
        ...
        PYTUNE.reader.$s.$story_titles_header.removeClass("NB-searching");
    ...
    update_indexing_progress: function(message) {
        ...
        if (message == "start") {
            ...
            _.each(feed_ids, function(feed_id) {
                var feed = PYTUNE.assets.get_feed(parseInt(feed_id, 10));
                feed.set('search_indexed', true);
            });
            this.show_indexing_tooltip(false);
            var indexed = PYTUNE.assets.feeds.search_indexed();
            var total = PYTUNE.assets.feeds.length;
            progress = Math.ceil(indexed / total * 100);
            PYTUNE.utils.attach_loading_gradient($input, progress);
        }
    },
    ...
    focus_search: function() {
        if (!PYTUNE.reader.flags.searching || !PYTUNE.reader.flags.search) {
            PYTUNE.reader.flags.searching = true;
            PYTUNE.reader.flags.search = "";
        }
        PYTUNE.reader.$s.$story_titles_header.addClass("NB-searching");
    },
    ...
    blur_search: function() {
        ...
        if (query.length == 0) {
            PYTUNE.reader.flags.searching = false;
            PYTUNE.reader.$s.$story_titles_header.removeClass("NB-searching");
    ...
    retry: function() {
        ...
        PYTUNE.reader.flags.search = null;
        this.search();
    },
    ...
    search: function() {
        ...
        if (query != PYTUNE.reader.flags.search) {
            PYTUNE.reader.flags.searching = true;
