PYTUNE.ReaderAddFeed = PYTUNE.ReaderPopover.extend({
    ...
    options: {
        'width': 380,
        'anchor': function() {
            return PYTUNE.reader.$s.$add_button;
        },
        'placement': 'top -left',
        offset: {
            top: 6,
    ...
    initialize: function(options) {
        ...
        PYTUNE.ReaderPopover.prototype.initialize.call(this);
            ...
            $.make('div', { className: 'NB-add-form' }, [
                $.make('div', { className: 'NB-fieldset NB-modal-submit' }, [
                    ...
                    $.make('div', { className: 'NB-group NB-add-site' }, [
                        PYTUNE.utils.make_folders(this.options.folder_title),
                ...
                $.make('div', { className: 'NB-fieldset NB-anonymous-ok NB-modal-submit NB-hidden' }, [
                    ...
                    $.make('div', { className: 'NB-fieldset-fields' }, [
                        $.make('div', { className: 'NB-add-import-button NB-modal-submit-green NB-modal-submit-button' }, [
                            ...
                            $.make('img', { className: 'NB-add-google-reader-arrow', src: PYTUNE.Globals['MEDIA_URL']+'img/icons/silk/arrow_right.png' })
                        ]),
                        $.make('div', { className: 'NB-add-danger' }, (PYTUNE.Globals.is_authenticated && _.size(this.model.feeds) > 0 && [
                            $.make('img', { src: PYTUNE.Globals['MEDIA_URL']+'img/icons/silk/server_go.png' }),
    ...
    setup_autocomplete: function() {
        ...
        $add.autocomplete({
            ...
            var feed = new PYTUNE.Models.Feed(item);
            return $.make('li', [
                $.make('a', [
    ...
    post_save_add_url: function(e, data) {
        ...
        if (data.code > 0) {
            PYTUNE.assets.load_feeds(function() {
                if (data.feed) {
                    PYTUNE.reader.open_feed(data.feed.id);
                }
            });
            PYTUNE.reader.load_recommended_feed();
            PYTUNE.reader.handle_mouse_indicator_hover();
            $submit.text('Added!');
            this.close();
    ...
    post_save_add_folder: function(e, data) {
        ...
        if (data.code > 0) {
            ...
            PYTUNE.assets.load_feeds(_.bind(function() {
                var $folders = PYTUNE.utils.make_folders($folder.val());
