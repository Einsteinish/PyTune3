...
for collection in collections:
    i = 0
    latest_item_id = 0
    total = db03.pytune[collection].count()
    items = db03.pytune[collection].find(sort=[('_id', pymongo.ASCENDING)])
    for item in items:
        if item.get('_id') != latest_item_id:
            ...
            sys.stdout.flush()
        db02.pytune[collection].insert(item)
        i += 1
...
for f, feed in enumerate(feeds):
    ...
    items = db03.pytune.stories.find({'story_feed_id': feed.id, 'story_date': {'$gte': latest_story_date}})
    for item in items:
        db02.pytune.stories.insert(item)
        total_inserted += 1
        feed_inserted += 1
    if feed_inserted:
