PYTUNE.Views.SocialProfileBadge = Backbone.View.extend({
    ...
    render: function() {
                ...
                $.make('td', { className: 'NB-profile-badge-info' }, [
                    ...
                    (_.isNumber(profile.get('shared_stories_count')) && 
                     $.make('div', { className: 'NB-profile-badge-stats' }, [
                        ...
                        ' &middot; ',
                        $.make('a', { href: profile.pytuneblog_url(), target: "_blank", className: "NB-profile-badge-pytuneblog-link NB-splash-link" }, profile.pytuneblog_url().replace('http://', '')),
                        (this.model.get('following_you') && $.make('span', [
                            ' &middot; ',
                            $.make('div', { className: 'NB-profile-badge-following-you' }, 'Follows you')
                        ])),
                        (PYTUNE.Globals.is_admin && $.make('span', [
                            ' &middot; ',
        ...
        } else if (PYTUNE.reader.model.user_profile.get('user_id') == profile.get('user_id')) {
            $actions = $.make('div', { className: 'NB-profile-badge-action-buttons' }, [
                $.make('div', { 
    ...
    follow_user: function() {
        ...
        PYTUNE.assets.follow_user(this.model.get('user_id'), _.bind(function(data) {
    ...
    unfollow_user: function() {
        this.$('.NB-loading').addClass('NB-active');
        PYTUNE.reader.model.unfollow_user(this.model.get('user_id'), _.bind(function(data, unfollow_user) {
            this.$('.NB-loading').removeClass('NB-active');
    ...
    approve_user: function() {
        this.$('.NB-loading').addClass('NB-active');
        PYTUNE.assets.approve_follower(this.model.get('user_id'), _.bind(function(data) {
            this.$('.NB-loading').removeClass('NB-active');
    ...
    ignore_user: function() {
        this.$('.NB-loading').addClass('NB-active');
        PYTUNE.assets.ignore_follower(this.model.get('user_id'), _.bind(function(data) {
            this.$('.NB-loading').removeClass('NB-active');
    ...
    preview_user: function() {
        ...
        var open_preview = _.bind(function() {
            ...
            var socialsub = PYTUNE.reader.model.add_social_feed(this.model);
            PYTUNE.reader.load_social_feed_in_tryfeed_view(socialsub);
        }, this);
    ...
    open_profile: function() {
        ...
        PYTUNE.reader.model.add_user_profiles([this.model]);
        
        if ($('.NB-modal').is(':visible')) {
            $.modal.close(function() {
                PYTUNE.reader.open_social_profile_modal(user_id);
            });
        } else {
            PYTUNE.reader.open_social_profile_modal(user_id);
        }
    ...
    open_user_admin: function() {
        PYTUNE.reader.open_user_admin_modal({user: this.model});
    }
